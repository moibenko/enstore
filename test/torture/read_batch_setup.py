...
if __name__ == '__main__':
    ...
    if make_copy:
        for node in nodes:
            ...
            print(
                "sending", "scp ~/enstore/test/torture/read_test.sh %s@%s:~/bin" %
                (user, node))
            os.system(
                "scp ~/enstore/test/torture/read_test.sh %s@%s:~/bin" %
                (user, node))
    ...
    for node in nodes:
        print(
            "sending",
            'ssh %s@%s "cd ~/bin; ~/bin/read_test.sh -b %s -c %s -w %s > read_%s.out 2>&1&"' %
            (user,
             node,
             b_list,
             count,
             ...
             pid))
        os.system(
            'ssh %s@%s "cd ~/bin; . /usr/local/etc/setups.sh; ~/bin/read_test.sh -b %s -c %s -w %s > read_%s.out 2>&1&"' %
            (user, node, b_list, count, works, pid))
        time.sleep(1)
