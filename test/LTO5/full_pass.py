...
def full_pass(i, job_config):
    ...
    time_to_sleep_between_passes = int(
        job_config.get('mover').get(
            'dismount_delay', 30)) * 3
    for i in range(number_of_full_passes):
        for volume in volumes:
            if os.path.exists(STOP_FILE):
                return 0
            print_message(
                "Starting pass %d of %d on volume %s" %
                (i + 1, number_of_full_passes, volume))
            ...
            if rc:
                print_error(
                    "volume assert of %s failed, pass %d of %d" %
                    (volume, i, number_of_full_passes))
                return 1
        print_message(
            "Completed full pass %d of %d on %d volumes, sleeping %d seconds" %
            (i + 1, number_of_full_passes, len(volumes), time_to_sleep_between_passes,))
