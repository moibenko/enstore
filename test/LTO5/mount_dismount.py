...
def mount_dismount(i, job_config):
    ...
    #
    Q1 = "select f.pnfs_path  from file f, volume v where f.volume=v.id and v.label='%s' and f.deleted='n' order by f.location_cookie" % (
        volume,)
    res = db.query(Q1)
    file_list = []
    done_files = []
    ...
    print_message(
        "Starting mount/dismount tape test for volume %s " %
        (volume,))
    ...
    for i in range(number_of_mounts_dismounts):
        if check_stop_file():
            return 0
        print_message(
            "Starting mount %d of %d" %
            (i, number_of_mounts_dismounts,))
        ...
        while mover_state != 'IDLE' and tries < max_tries:
            ...
            tries += 1
            print_message(
                "Mover is in state '%s'. Sleeping 40 seconds before querying its status again" %
                (mover_state,))
            time.sleep(40)
            status = moverClient.status(3, 2)
            if e_errors.is_ok(status['status'][0]):
        ...
        if mover_state != 'IDLE':
            ...
            while ticket['status'][0] in [e_errors.MC_QUEUE_FULL]:
                print_error(
                    "Failed to dismount tape %s, %d of %d %s" %
                    (volume, i, number_of_mounts_dismounts, str(
                        ticket['status'])))
                ...
                if check_stop_file():
                    return 0
                if ticket['status'][0] != e_errors.OK and ticket['status'][0] not in [
                        e_errors.MC_QUEUE_FULL]:
                    print_error(
                        "Failed to dismount tape %s, %d of %d %s" %
                        (volume, i, number_of_mounts_dismounts, str(
                            ticket['status'])))
