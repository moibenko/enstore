...
class UdpPeer(object):
    ...
    def _sendNaks(self):
        ...
        t = time.time()
        self.LastNakTime = t
        for peer in list(self.Links.keys()):
            lnk = self.Links[peer]
            if t > lnk.LastSeen + self.CloseTimeout:
                del self.Links[peer]
                raise_(UdpDisconnect, peer)
            else:
                lnk.sendNak()
    ...
    def recvfrom(self, max):
        ...
        while int(time.time()) - t0 < self.FailTimeout:
            ...
            data = lnk.getData()
            if data != 0:
                return data, lnk.Peer

        self._sendNaks()
        raise AttributeError(UdpTimeout)
