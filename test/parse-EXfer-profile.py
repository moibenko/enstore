...
for kind in "rsel", "read", "wsel", "write":
    ...
    while 1:
        ...
        if tokens[0] == first:
            ...
            nbytes = int(tokens[5])
            Mbytespersec = nbytes / delta / 1024. / 1024. * 1000000
            o.write(
                "%i %f %f %f %f \n" %
                (count, t, delta, nbytes, Mbytespersec))

    if end == -1:
        end = int(s) + int(u) * .000001
    o.close()
    f.close()
...
print(' set log y')
print(
    ' plot "%srsel" using 1:3, "%sread" using 1:3, "%swsel" using 1:3, "%swrite" using 1:3' %
