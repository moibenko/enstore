...
# format_exception - when unecpected error occurs
import traceback
...
if uname[0] == 'Linux':
    ...
    if sts:
        print(
            'unexpected error reading Linux scsi controller %s info' %
            tape_ctlr)
        sys.exit(1)
        pass

    tape_drv = proc_path[0].split('/')[-2]
    if tape_drv == 'aic7xxx':
...
def online_test(tape_dev):
    ...
    sts = FTT.get_stats()
    if sts['bloc_loc'] != '0':
        raise AttributeError('unexpected bloc_loc')

    print('    get_mode')
    mode = FTT.get_mode()
    print('FTT.get_mode() returned:', mode)
    if mode['blocksize'] != 0:
        raise AttributeError('unexpected blocksize')
...
# dev_name)
    ...
    for blksiz in no_except_blksz:
        ...
        if ss != ' ' * 80:
            print(
                'read of 80 bytes returned %s bytes (data not checked)' %
                len(ss))
            raise AttributeError('unexpected FTT.read return value')
        pass
...
def offline_test(tape_dev):
    ...
    try:
        ...
        if tape_drv == 'ncr53c8xx' and value.args[4] == FTT.FTT_EBLANK:
            ...
            print(
                'ftt return %s when running no top of the %s driver' %
                (value.args[4], tape_drv))
            pass
        print()
        pass
...
def off_to_online_test(tape_dev):
    ...
    while sts['ONLINE'] == 0:
        ...
        pass
    if sts['ONLINE'] != 1:
        raise AttributeError("unexpected 'ONLINE' value %s" % sts['ONLINE'])
