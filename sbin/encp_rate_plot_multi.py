...
def main():
    ...
    if not os.path.exists("%s/%s" % (web_dir, ENCP_RATE)):
        ...
        subprocess.call("cp ${ENSTORE_DIR}/etc/*.gif %s/%s" % (web_dir, ENCP_RATE), shell=True)
    ...
    out, err = enstore_functions2.shell_command(cmd)
