...
def findChild(ppid, fromNode, toNode):
    ...
    while num < len(a) - 1:
        b = a[num].split()
        if b[0] == str(ppid) and a[num].find('enrsh %s "%s -l %s -H %s"' %
    ...
    while num < len(testdict.keys()):
        ...
        if (paramdict['fFlg'] == FALSE) and (fromNode == toNode):
            ...
            if testdict[name] < 0:
                ...
                if isAlive.lower().find("zombie") < 0:
                    os.system(
                    ...
                    tempresltdict[name] = tempresltdict[name].split("\n")
                    if tempresltdict[name][0].find(
                            # SEE IF TEST WAS EVER DONE
                            "TCP STREAM TEST") < 0:
                        resultdict[name] = "-2"
                    else:
...
def testNodes():
    ...
    if numTests < 3:
        if numTstGrps == 2 and paramdict['tFlg'] and not paramdict['fFlg']:
            testTime = int(paramdict['testtime']) + int(cuTime)
        else:
            testTime = numTests * \
                int(paramdict['testtime']) + int(cuTime)
    else:
        if paramdict['tFlg']:
            testTime = numTstGrps * \
                int(paramdict['testtime']) + int(cuTime)
    ...
    while grpSeqNum < numTstGrps:
        ...
        if prevTstNum != testNum:                          # DON'T PAUSE FOR TEST IF TEST WASN'T DONE
            ...
            time.sleep(int(paramdict['testtime']))
...
def printResults():
    ...
    while lpCntr < len(
        ...
        if len(testList[lpCntr]) < 7:
            ...
            colWidth = len(testList[lpCntr]) + 1
        sys.stdout.write(testList[lpCntr].rjust(colWidth))
        lpCntr = lpCntr + 1
    sys.stdout.write("\n")
    lpCntr = 0
    while lpCntr < hdrLen:
    ...
    while fromPtr < len(
        ...
        sys.stdout.write(testList[fromPtr].ljust(strtColWidth))
        toPtr = 0
        while toPtr < len(testList):  # PRINTS THE TEST RESULTS
            ...
            sys.stdout.write(" %s" % " %s" %
                             resultdict["%s %s" %
                                        (testList[fromPtr], testList[toPtr])].rjust(colWidth))
            toPtr = toPtr + 1
        sys.stdout.write("\n")
        fromPtr = fromPtr + 1
    sys.stdout.write(
...
def getArgs(args):
    ...
    while num < len(optlist):
        if optlist[num][0] == '-h':
            ...
            if optlist[num][1].lower() == "n":
                paramdict['fFlg'] = FALSE
            elif not optlist[num][1].lower() == "y":
                sys.stderr.write(
