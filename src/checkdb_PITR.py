...
def start_postmaster(db_path):
    ...
    if pid:
        ...
        # You could change the configuration and set it to "off",
        # then PostgreSQL will continue to work as before.
        cmd = "postmaster -c archive_mode=off -c hot_standby=off -D %s &" % (
            db_path)
        os.system(cmd)
        time.sleep(15)
        rc = checkPostgres()
...
"""
    ...
    list(map(lambda x: x.close(), list(output_files.values())))
    list(map(lambda x: x.close(), list(recent_output_files.values())))
