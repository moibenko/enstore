...
def get_entvrc(csc=None):
    ...
    try:
        ...
        for line in entvrc_data:
            ...
            for system_name, csc_addr in list(config_servers.items()):
...
def get_system_info_from_entvrc(tk, entvrc_dict, system_name=None):
    ...
    if system_name:
        for config_host, tmp_entvrc_info in \
                list(entvrc_dict.get('system_info', {}).items()):
...
def handle_status(mover, status):
    if mover == "inquisitor":
        ...
        for mover_name, outage in list(outage_dict.items()):
        ...
        for mover_name, reason in list(offline_dict.items()):
            if not enstore_functions2.is_mover(mover_name):
                # This isn't a scheduled down mover.
                continue
            send_error_list[mover_name] = "error %s %s" % \
                                          (mover_name.split(".")[0], reason)
        return list(send_error_list.values())
...
def drop_stale_status_requests():
    ...
    for system_name, send_request_dict_copy in list(all_send_request_dict_copy.items()):
        for tx_id in list(send_request_dict_copy.keys()):
...
def setup_movers(system_name, display, intf):
    ...
    skip_library_list = intf.dont_show.split(",")
    ...
    movers_command = " ".join(["movers"] + mover_list)
    ...
    if intf and intf.generate_messages_file:
        for mover in mover_list:
            idle_command = " ".join(["state", mover, "IDLE"])
...
#                    erc.sock.close()
            ...
            if u.get_tsd().socket in readable:
                process_udp(u, tkinter._tkinter.READABLE)
            ...
            for erc in get_ercs():
                if erc in readable:
                    process_erc(erc, tkinter._tkinter.READABLE)
    ...
    # We will get all of the info from the event relay.
        ...
        if erc:
            ...
            if not intf.threaded:
                j = tkinter.Tk()
                j.createfilehandler(erc, tkinter._tkinter.READABLE,
                                    process_erc)
                j.destroy()
                del j
        ...
        if not intf.threaded:
            try:
                j = tkinter.Tk()
                j.createfilehandler(u, tkinter._tkinter.READABLE, process_udp)
                j.destroy()
                del j
            except Exception as e:
                Trace.trace(1, "Exception in  setup_networking: %s." % (e,))
...
def process_udp(local_udp_client, mask):
    ...
    for system_name in list(send_request_dict_copy.keys()):
        commands = []
        for tx_id in list(send_request_dict_copy[system_name].keys()):
            try:
                mstatus = u.recv_deferred(tx_id, 0.0)
...
def create_menubar(menu_defaults, system_defaults, master, intf):
    ...
    # Menubar attributes.
    master.entv_menubar = tkinter.Menu(master)
    # Options menu.
...
def systems_enabled_statistics(master):
    ...
    enabled = 0
    disabled = 0
    for boolean in list(ese.values()):
        if boolean.get():
            enabled = enabled + 1
        else:
            disabled = disabled + 1

    return (enabled, disabled, len(list(ese.keys())))


def configurated_systems(master):
    ese = getattr(master, "enstore_systems_enabled", {})
    return list(ese.keys())
...
def toggle_connection_color():
    ...
    for display in displays:
        ...
        for connection in list(display.connections.values()):
...
def toggle_systems_enabled():
    ...
    for system_name, enabled in \
            list(master_windowframe.enstore_systems_enabled.items()):
...
def main(intf):
    ...
    if intf.messages_file:
        ...
        for system_name, current_csc in list(cscs.items()):
        ...
        entvrc_dict = get_entvrc(csc=None)
        for config_hostname in list(entvrc_dict['system_info'].keys()):
...
def test_for_threading(intf):
    ...
    try:
        ...
        #tkinter._tkinter.createfilehandler(u, tkinter.READABLE, process_udp)
        #tkinter._tkinter.deletefilehandler(u)
