        ...
        if moverd:
            ...
            if enstore_constants.STATE in moverd and \
               moverd[enstore_constants.STATE]:
                ...
                words = moverd[enstore_constants.STATE].split()
            ...
            for k in md:
                ...
                if isinstance(md[k], dict) and enstore_constants.STATE in md[k] and \
                   md[k][enstore_constants.STATE]:
                    # get the first word of the mover state, we will use this to
                    # tell if this is a bad state or not
                    words = md[k][enstore_constants.STATE].split()
...
class EnSysStatusPage(EnBaseHtmlDoc):
    ...
    def lm_rows(self, lm):
        ...
        if lm_d and lm_d[enstore_constants.STATUS][0].find(NBSP) == -1:
        ...
        if enstore_constants.LMSTATE in lm_d and \
           isinstance(lm_d[enstore_constants.LMSTATE], bytes):
            words = lm_d[enstore_constants.LMSTATE].split()
        else:
            words = ["",]
    ...
    def mover_row(self, server):
        ...
        if enstore_constants.STATE in mover_d and \
           mover_d[enstore_constants.STATE]:
            ...
            # if we are updating the web page faster that receiving the new
            # info, then we already have a correct status
            if mover_d[enstore_constants.STATUS][0].find(NBSP) == -1 and \
            ...
            words = mover_d[enstore_constants.STATE].split()
            if words[0] in BAD_MOVER_STATES:
                return self.alive_row(server,
        ...
        for k in m_d:
            if isinstance(m_d[k], dict) and enstore_constants.STATE in m_d[k] and \
               m_d[k][enstore_constants.STATE]:
                ...
                # if we are updating the web page faster that receiving the new
                # info, then we already have a correct status
                if m_d[enstore_constants.STATUS][0].find(NBSP) == -1 and \
                ...
                # to tell if this is a bad state or not
                words = m_d[k][enstore_constants.STATE].split()
                if words[0] in BAD_MIGRATOR_STATES:
                    return self.alive_row(server,
...
# too general     "USER ERROR"                                : "USERERROR",
    ...
    def __init__(self, refresh=120, system_tag=""):
        ...
        self.source_server = THE_INQUISITOR
        self.description = ""
        self.error_keys = list(self.error_text.keys())
        ...
        while j > 0:
            ...
            if len(row) != 4:
                ...
                for ekey in self.error_keys:
                    if row[3].find(self.error_text[ekey]) != -1:
                        # found a match
                        etxt = ekey
                        break
                else:
...
class EnAlarmPage(EnBaseHtmlDoc):
    ...
    def sort_by_latest_time(self, alarms):
        ...
        # alarmed to earliest
        keys = list(alarms.keys())
        # build array
        latest_time_keys = []
        for key in keys:
            latest_time_keys.append([alarms[key].timedate_last, key])
        else:
            if len(latest_time_keys) > 1:
                #latest_time_keys.sort(key=get_time, reverse = True)
                latest_time_keys.sort(reverse = True)
    ...
    def body(self, alarms, web_host):
        ...
        form = HTMLgen.Form("/cgi-bin/enstore_alarm_cgi")
...
class EnPlotPage(EnBaseHtmlDoc):
    ...
    def find_label(self, text):
        ...
        for file_label in PLOT_INFO:
            index = text.find(file_label[0])
            if not index == -1:
                # this is a match
                if file_label[0] == enstore_constants.UTIL_FILE:
                    # fix up the label
                    p_type = text.split("_", 1)
                    ...
                    just_mover = text[index:].replace(file_label[0], "")
                    # get rid of _stamp.jpg too
                    index = just_mover.find(enstore_constants.STAMP)
                    ...
                    just_sg = text[index:].replace(file_label[0], "")
                    index = just_sg.find(enstore_constants.STAMP)

                    if just_sg[1:index] == "":
                        return file_label[1]
                    tmp = just_sg.split("_", 1)[1]
                    tmp1 = tmp.replace(".", " ")
                    tmp2 = tmp1.replace("log", "Log Scale")
                    index = tmp2.find(enstore_constants.STAMP)
    ...
    def find_ps_file(self, jpg_file, pss):
        ...
        ps_file = ""
        if pss:
            ps_file = jpg_file.replace(enstore_constants.JPG,
    ...
    def add_stamp(self, jpgs, stamps, pss, trs, trps, trps2):
        ...
        if stamps:
            stamp = stamps.pop(0)
            jpg_file = stamp[0].replace(enstore_constants.STAMP, "")
...
class EnSaagPage(EnBaseHtmlDoc):
    ...
    def make_overall_table(self, enstat_d, other_d, medstat_d, alarms_d, outage_d,
        ...
        for item in med_l:
            ...
            self.add_to_row(tr, list(item.keys())[0], item, outage_d, offline_d)
...
class EnSGIngestPage(EnBaseHtmlDoc):
    ...
    def body(self):
        ...
        for lib in libs:
            ...
            rows = int(len(libraries[lib]) / 6)  # 6 entries per raw
            if int(len(libraries[lib]) % 6):
                rows = rows + 1
