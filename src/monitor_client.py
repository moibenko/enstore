...
class MonitorServerClient(generic_client.GenericClient):
    ...
    def _send_measurement(self, ticket):
        try:
            ...
            x = {'status': (e_errors.TIMEDOUT, str(e))}
        return x
    ...
    def monitor_one_interface(self, server_address, transfer):
        ...
        try:
            ...
            reply = {}
            reply['status'] = (e_errors.TIMEDOUT, str(e))
            reply['elapsed'] = self.timeout * 10
            reply['block_count'] = 0
    ...
    def alive(self, server, rcv_timeout=0, tries=0):
        ...
        try:
            ...
            Trace.trace(14, '%s %s' % (message, str(e)))
            x = {'status': (e_errors.TIMEDOUT, '%s %s' % (message, str(e)))}
        return x
...
def get_all_ips(config_host, config_port, csc):
    ...
    ip_dict = {}
    for k in list(server_dict.keys()):
    ...
    return list(ip_dict.keys())  # keys of the dict is a tuple of IPs
