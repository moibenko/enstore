...
#
    ...
    def start(self):
        ...
        self.qpid_command_receiver = self.qpid_client.add_receiver(
            "mw_qpid_interface", self.queue_in)

        # start servers in separate threads
        self.srv_thread = threading.Thread(
            target=self.serve_qpid, name="Request Server", args=[
                self.qpid_client.rcv_default])
        self.srv_thread.start()

        self.cmd_srv_thread = threading.Thread(
            target=self.serve_qpid, name="MD Commmand Server", args=[
                self.qpid_command_receiver])
