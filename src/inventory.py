...
def get_vol_filenames(output_dir):
    if output_dir.find("/dev/stdout") != -1:
...
def format_storage_size(size_in_bytes, mode="GB"):
    ...
    count = 0
    while int(abs(volume_size) / 1024) > 0:
...
def os_write(file_descriptor, out_string):
    """
    wrapper around os.write
    :type file_descriptor:
    : arg file_descriptor: file descriptor returned by os.open()
    : type out_string: :obj:`str`
    : arg out_string: string to write
    """
    if isinstance(out_string, str):
        out_string = out_string.encode()
    os.write(file_descriptor, out_string)
...
def print_header(volume, fd):
    ...
    os_write(fd, out_string)

    if volume['modification_time'] == -1:
        os_write(fd, "Last accessed on: Never\n")
    else:
...
def print_footer(volume, s_i, fd):
    ...
    os_write(fd, '%d/%d/%d/%d (active/deleted/unknown/total) bytes\n\n' %
...
def print_volume_quotas_status(
    ...
    for key in list(volume_quotas.keys()):
    ...
    if 'libraries' in quotas:
        libraries = list(quotas['libraries'].keys())
    else:
        libraries = []
    quotas = list(volume_quotas.keys())
...
def print_total_bytes_on_tape(volume_sums, output_file):
    summation = 0
    for line in list(volume_sums.keys()):
...
def verify_volume_sizes(this_volume_data, volume, volume_sizes):
    ...
    try:
        for line in this_volume_data:
            ...
            sum_size = sum_size + int(split_string[2])

            # Count the sum of all the file sizes of deleted and non-deleted
            # file seperately.
            if split_string[4] == "yes":
                deleted_size = deleted_size + int(split_string[2])
            elif split_string[4] == "no":
                non_deleted_size = non_deleted_size + int(split_string[2])
    except IndexError:
...
def verify_volume_quotas(volume_data, volume, volumes_allocated):
    ...
    storage_group = volume['volume_family'].split(".")[0]
...
def inventory(output_dir, cache_dir):
    ...
    if os.access(volume_summary_cache_file, os.F_OK):
        print("CACHE FILE", volume_summary_cache_file)
        sum_f = open(volume_summary_cache_file, 'rb')
        vol_sum = pickle.load(sum_f)
        sum_f.close()
    else:
        vol_sum = {}
    ...
    for vk in list(vol_db.keys()):
        ...
        if vsum and int(vsum['last']) == int(vv['modification_time']):
            skipped = True
        ...
        # last access time updated.
            ...
            for f in res:  # bfids:
                os_write(fd_output, "%10s %15s %15s %22s %7s %s\n" %
                         (f.get('label', "unknown"),
                          f.get('bfid', "unknown"),
                          f.get('size', "unknown"),
        ...
        # check quota stuff
        storage_group = vv['volume_family'].split('.')[0]
        library = vv['library']
        try:
            quota = quotas['libraries'][library][storage_group]
        except BaseException:
    ...
    sum_f = open(volume_summary_cache_file, 'wb')
    #if isinstance(vol_sum, str):
    #    vol_sum = vol_sum.encode()
    pickle.dump(vol_sum, sum_f, protocol=0)
    sum_f.close()
    ...
    os.system(
        ...
        '.html')
    os.system(
        'sed -e "s/<font color=#FF0000>//g; s/<\\/font>//g; s/<blink>//g; s/<\\/blink>//g" ' +
        volumes_defined_file +
        '.html > ' +
        volumes_defined_file)
...
def inventory_dirs():
    csc = configuration_client.ConfigurationClient()
    inven = csc.get('inventory', timeout=15, retry=3)
    if not 'status' in list(inven.keys()):
...
#    print "inventory_rcp_dir", inventory_rcp_dir
    ...
    if inventory_rcp_dir:
        if output_dir.find("/dev/stdout") == -1:
            ...
            if this_host in inventory_rcp_dir:
                inventory_cp_dir = inventory_rcp_dir.split(':')[1]
                print(
                "cp %s %s" %
                (os.path.join(
                    output_dir,
