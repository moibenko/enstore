...
class FTTDriver(generic_driver.Driver):
    ...
    e_errors.INFO, "ftt open dev: %s %s: reopening read-only" %
     (detail, detail.value))
                    ...
                    # Trace.trace(42, "ftt.close() done")
                    # Trace.trace(42,
                    # "ftt.open(%s,%s)"%(self.device,ftt.RDONLY))
                    self.ftt = ftt.open(self.device, ftt.RDONLY)
                    self.mode = 0
                    # Trace.trace(42, "%s=ftt.open(%s,%s)
                    # done"%(self.ftt,self.device,ftt.RDONLY))
                    ...
                    try:
                        ...
                        Trace.log(e_errors.ERROR, "ftt close dev error: %s %s" %
                                  (detail, detail.errno))
                    except:
                        pass
...
##            raise ValueError, "file not open for reading"
    ...
    def eject(self):
        ...
        for retry in (0, 1, 2):
            ...
            if not s:
                ...
                if r.find("Input/output error") >= 0:
                ...
                elif r.find("No medium found") >= 0: # there is no media no need to offline
                    ok = 1
        if not ok:
    ...
    def check_addtl_hdrs(self, nbytes, buf, mode):
        ...
        #           UVL1 - UVL9
        bytes_l = list(self.labels.keys())
        for abytes in bytes_l:
            if nbytes > abytes:
                if buf[abytes:abytes+4] not in self.labels[abytes]:
                    # we found a label in a spot where it should not be
                    return {0:"READ_ERR-at-%s"%(abytes,),
                            1:"WRITE_ERR-at-%s"%(abytes,)}[mode]
        return None
    ...
    def verify_label(self, volume_label=None, mode=0, expected_length=1440):
        ...
        if volume_label:
            volume_label = volume_label.encode()
        buf=expected_length*b' '
        try:
