    ...
    def __init__(self, bfid=None, nsid=None, path=None, libraries=None):
        ...
        if [bfid, nsid, path, libraries].count(None) != 0:
            raise e_errors.EnstoreError(
                None,
                "need bfid, nsid, path, libraries",
                e_errors.WRONGPARAMETER)

        if not isinstance(libraries, list):
            raise e_errors.EnstoreError(
                None, "libraries must be a list", e_errors.WRONGPARAMETER)
        self.d = {"bfid": bfid,
                  "nsid": nsid,
                  "path": path,
                  "libraries": libraries,
...
class FileListItemWithCRC:
    def __init__(self, bfid=None, nsid=None,
                 path=None, libraries=None, crc=None):

        if [bfid, nsid, path, libraries, crc].count(None) != 0:
            raise e_errors.EnstoreError(
                None,
                "need bfid, nsid, path, libraries, crc",
                e_errors.WRONGPARAMETER)
...
class FileList:
    ...
    def _append(self, item, data_size=0):
        self.state = FILLING
    ...
    def append(self, item, data_size=0):
        if isinstance(item, FileListItem):
            self._append(item, data_size)
        else:
            raise e_errors.EnstoreError(
                None, "item must be FileListItem", e_errors.WRONGPARAMETER)
...
class FileListWithCRC(FileList):
    ...
    def append(self, item, data_size=0):
        if isinstance(item, FileListItemWithCRC):
            ...
            raise e_errors.EnstoreError(
                None,
                "item must be FileListItemWithCRC",
                e_errors.WRONGPARAMETER)
...
if __name__ == "__main__":   # pragma: no cover
    ...
    ic = FileListItemWithCRC(bfid="CDMS0001", nsid="0DAF0001", path="/diska/aaa/f1",
                             libraries=["lib1", "lib2"], crc=3020422051)
    try:
