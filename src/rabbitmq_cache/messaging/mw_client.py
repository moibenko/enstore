...
class MWCommand(EnqMessage):
    ...
    def __init__(self, mtype=None, content=None, **kwargs):
        if [mtype, content].count(None) != 0:
            raise e_errors.EnstoreError(
                None,
                "type or file list is undefined",
                e_errors.WRONGPARAMETER)
        super().__init__(message_type=mtype, content=content, **kwargs)
...
class MWCPurge(MWCommand):
    ...
    def __init__(self, file_list=None, **kwargs):
        ...
        MWCommand.__init__(self, mtype=mt.MWC_PURGE, content=c, **kwargs)
...
class MWCArchive(MWCommand):
    ...
    def __init__(self, file_list=None, **kwargs):
        c = {"file_list": file_list}
        MWCommand.__init__(self, mtype=mt.MWC_ARCHIVE, content=c, **kwargs)
...
class MWCStage(MWCommand):
    ...
    def __init__(self, file_list=None, **kwargs):
        c = {"file_list": file_list}
        MWCommand.__init__(self, mtype=mt.MWC_STAGE, content=c, **kwargs)


class MWCStatus(MWCommand):
    """ Message: Migration Worker Status Command
    """
    # Hmm, request_id supposed to be correlation_id of message we are trying
    # to track ...
    ...
    def __init__(self, request_id=None, **kwargs):
        if request_id is None:
            raise e_errors.EnstoreError(
                None,
                "missing 'request_id' argument to MWCStatus() constructor",
                e_errors.WRONGPARAMETER)
        c = {"request_id": request_id}
        MWCommand.__init__(self, mtype=mt.MWC_STATUS, content=c, **kwargs)
...
class MWReply(EnqMessage):
    ...
    """
#    def __init__(self, mtype=None, orig_msg = None, content = None, **kwargs ):

    def __init__(self, mtype=None, orig_msg=None, **kwargs):
        if [mtype, orig_msg].count(None) != 0:
            raise e_errors.EnstoreError(None, "missing 'mtype' or 'orig_msg' argument to MWReply() constructor",
                                        e_errors.WRONGPARAMETER)
        # if not kwargs.has_key("content") and type is not mt.MWR_CONFIRMATION
        # : # logical equivalent
        if not ("content" in kwargs or mtype is mt.MWR_CONFIRMATION):
...
#                                        e_errors.WRONGPARAMETER)
        ...
        EnqMessage.__init__(self, mtype=mtype, **kwargs)
#        EnqMessage.__init__(self, mtype=mtype, content=content, **kwargs)
        # @todo: fix, set correlation_id in args to constructor
        try:
            self.correlation_id = orig_msg.correlation_id  # reset correlation id
        except BaseException as e:
            print("mw_reply exception {}".format(e))
            pass
