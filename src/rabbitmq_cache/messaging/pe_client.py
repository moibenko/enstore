...
from rabbitmq_cache.messaging.enq_message import EnqMessage
...
class PEEvent(EnqMessage):
    ...
    def __init__(self, message_type=None, content=None):
        if [message_type, content].count(None) != 0:
            raise e_errors.EnstoreError(
                None,
                "type or content is undefined",
                e_errors.WRONGPARAMETER)
        durable = True
        EnqMessage.__init__(self, message_type=message_type, content=content, durable=durable)
...
class EvtFileDeleted(PEEvent):
    ...
    def __init__(self, content=None):
        PEEvent.__init__(self, message_type=mt.FILE_DELETED, content=content)
...
class EvtCacheMissed(PEEvent):
    ...
    def __init__(self, content=None):
        PEEvent.__init__(self, message_type=mt.CACHE_MISSED, content=content)
...
class EvtCachePurged(PEEvent):
    ...
    def __init__(self, content=None):
        PEEvent.__init__(self, message_type=mt.CACHE_PURGED, content=content)
...
class EvtCacheWritten(PEEvent):
    ...
    def __init__(self, content=None):
        PEEvent.__init__(self, message_type=mt.CACHE_WRITTEN, content=content)
...
class EvtCacheStaged(PEEvent):
    ...
    def __init__(self, content=None):
        PEEvent.__init__(self, message_type=mt.CACHE_STAGED, content=content)
...
def _get_proto(ticket, vc_keys=None, fc_keys=None):
    ...
    if fc_keys is not None:
        if 'enstore' not in proto:
            proto['enstore'] = {}
        # copy directly to 'enstore' part of the ticket, flattening nested 'fc'
        # dictionary
        for k in fc_keys:
...
def evt_cache_written_fc(encp_ticket, fc_record):
    ...
    fc_ticket["vc"]["library"] = encp_ticket["fc"].get(
...
# ---
    vct = {
        ...
        'disk_library': 'DSK_lib'
    }
    fct = {
        # or shall it be taken from ticket = {'bfid':'GCMS129683447700000',}
        'bfid': 'GCMS129683447700000',
        'location_cookie': '0000_000000000_0000001',
        'pnfs_name0': '/pnfs/fs/usr/data/moibenko/d1/mover.py',
        'pnfsid': '0001000000000000000011F8',
        'size': 1234567890,
        'complete_crc': 3020422051,  # only in evt_cache_written_t, evt_cache_missed_t
        'deleted': 'no',
        'disk_library': 'DSK_lib'
    }
...
#  as of 11/04/2011 :
    ...
    encp_tk = {'status': ('ok', None),
               'r_a': (('131.225.13.37', 42424), 1, '131.225.13.37-42424-1320439860.181562-31985-47728652978352'),
               'work': 'set_pnfsid',
               'fc': {'size': 5189, 'pnfsvid': '', 'pnfsid': '0001000000000000000031F0',
                      ...
                      'complete_crc': 2338790700, 'original_library': 'LTO3GS', 'mover_type': 'DiskMover',
                      'file_family_width': 20, 'bfid': 'GCMS132043986900000', 'uid': 5744, 'sanity_cookie': (5189, 2338790700),
    ...
    fc_record = {'storage_group': 'ANM', 'pnfsvid': '', 'uid': 5744,
                 ...
                 'location_cookie': '/data/cache/496/3/0001000000000000000031F0', 'complete_crc': 2338790700,
                 'bfid': 'GCMS132043986900000',
                 'sanity_cookie': (5189, 2338790700), 'size': 5189,
                 'external_label': 'common:ANM.FF1.cpio_odc:2011-11-04T15:51:08Z',
                 'drive': 'dmsen06:/data/cache:0', 'wrapper': 'null', 'package_files_count': 0,
                 'active_package_files_count': -1, 'gid': 6209,
                 'pnfsid': '0001000000000000000031F0', 'archive_mod_time': None, 'disk_library': 'DSK_lib',
                 'cache_status': 'CACHED'}
# ---
...
# ---
    ...
    print("evt_cache_written_fc(): %s %s %s %s" % (ew_fc, dir(ew_fc), ew_fc.properties, ew_fc.content))
