...
import mw
import rabbitmq_cache as cache
import rabbitmq_cache.messaging.mw_client
...
import rabbitmq_cache.messaging.normalize_ticket as normalize_ticket
...
class Dispatcher(mw.MigrationWorker,
    ...
    def __init__(self, csc, auto_ack=True):
        ...
        self.dispatcher_configuration['server']['queue_in'] = self.queue_in_name
        ...
        self.dispatcher_configuration['amqp']['broker'] = self.csc.get(
            "amqp_broker")
        ...
        self.clustered_configuration = self.my_conf.get(
        ...
        # start_purging_disk_avalable - available space as a fraction of the
        # capacity
        ...
        # for clustered systems purge only
        libraries = self.my_conf.get("libraries_to_purge")
        ...
        mw.MigrationWorker.__init__(
            self, self.name, self.dispatcher_configuration)
        ...
        dispatching_worker.DispatchingWorker.__init__(self, (self.my_conf['hostip'],
                                                      self.my_conf['port']))
        self.logger, self.tracer = en_logging.set_logging(self.log_name)
    ...
    def reload_policy(self, ticket):
        try:
            ...
            ticket['status'] = (
                e_errors.ERROR, "Error loading policy for PE Server: %s" %
                (detail,))
    ...
    def show_id(self, ticket):
        if ticket['id'] not in self.migration_pool:
            ticket['status'] = (
                e_errors.KEYERROR, "No such id %s" %
                (ticket['id'],))
            self.reply_to_caller(ticket)
            return
        ticket['status'] = (e_errors.OK, None)
            ...
            for item in items:
                try:
                    ...
                    if requested_id and requested_id != id:
                        ...
                        self.move_to_migration_pool(
                            self.cache_written_pool, item)
    ...
    def move_to_purge_pool(self, item_to_move):
        ...
        try:
            ...
            Trace.log(
                e_errors.ERROR, "Error moving to purge pool: %s" %
                (detail,))
        if item_to_move in self.cache_purge_pool:
    ...
    def check_pools(self):
        ...
        for pool in (self.file_deleted_pool,
            ...
            for key in pool.keys():
                if pool[key].list_expired():
                    ...
                    Trace.trace(
                        10, "check pools passing to migration dispatcher %s" %
                        (key,))
                    self.move_to_migration_pool(pool, key)
                    self.md.start_migration(self.migration_pool, id)
        files_to_purge = []
        Trace.trace(
            10, "check_pools: purge_pool %s" %
            (self.cache_purge_pool,))
        ...
        for key, val in self.cache_purge_pool.items():
            if val.list_expired():
                ...
                # pass this list to Migration Dispatcher
                Trace.trace(
                    10, "check pools passing purge to migration dispatcher %s" %
                    (key,))
                ...
                rc = set_cache_status.set_cache_status(
                    self.fcc, files_to_purge)
                if rc['status'][0] != e_errors.OK:
                    Trace.log(
                        e_errors.ERROR, "set_cache_status failed %s" %
                        (rc,))
    ...
    def check_pools_thread(self):
        ...
        while not self.shutdown:
            ...
            try:
                ...
                Trace.log(
                    e_errors.ERROR, "Exception in check_pools %s" %
                    (detail,))
            time.sleep(10)
    ...
    def handle_cache_missed(self, message):
        Trace.trace(10, "handle_cache_missed received: {}".format(message.content))
        n_content = message.content['content']
        l_name = n_content['file']['name']
        if self.clustered_configuration:
            disk_library = n_content['enstore'].get('disk_library')
        else:
            disk_library = ""
        ...
        list_element = file_list.FileListItemWithCRC(bfid=n_content['enstore']['bfid'],
                                                     nsid=n_content['file']['id'],
                                                     path=n_content['file']['name'],
                                                     libraries=[
                                                         n_content['enstore']['vc']['library']],
                                                     crc=n_content['file']['complete_crc'])

        self.cache_missed_pool[l_name].append(list_element)
        # if this is a package trigger sending the list
        # to the migrator right away
        bfid = n_content['enstore']['bfid']

        Trace.trace(
            10, "handle_cache_missed getting bfid info for %s" %
            (bfid,))
        rec = self.fcc.bfid_info(bfid)
        if rec['status'][0] == e_errors.OK and n_content['enstore']['bfid'] == rec['package_id']:
    ...
    def handle_purge_cache(self, message):
        ...
        Trace.trace(
            10, "handle_purge_cache: message file list %s" %
            (list_of_files,))
        self._lock.acquire()
        try:
    ...
    def handle_cache_written(self, message):
        ...
        Trace.trace(
            10, "handle_cache_written content: %s" %
            (message.content,))
        Trace.trace(
            10, "handle_cache_written correlation_id: %s" %
            (message.correlation_id,))
        Trace.trace(10, "handle_cache_written type: %s" %
        ...
        Trace.trace(10, "NEW CONT {} {}".format(new_content,  type(new_content)))
        ...
        Trace.trace(
            10, "handle_cache_written new content: %s" %
            (new_content,))
        policy = self.policy_selector.match_found_pe(new_content['enstore'])
        if policy:
            # check if the file list exists
            Trace.trace(
                10, "handle_cache_written pool: %s" %
                (self.cache_written_pool.keys(),))
            ...
            list_element = file_list.FileListItemWithCRC(bfid=new_content['enstore']['bfid'],
                                                         ...
                                                         libraries=[
                new_content['enstore']['vc']['library']],
                crc=new_content['enstore']['complete_crc'])

            self.cache_written_pool[policy['policy']].append(
                list_element, new_content['enstore']['file_size'])
            if self.cache_written_pool[policy['policy']].full:
                # pass this list to Migration Dispatcher
                id = self.cache_written_pool[policy['policy']].list_id
                Trace.trace(
                    10, "handle_cache_written passing to migration dispatcher")
                self.move_to_migration_pool(
                    self.cache_written_pool, policy['policy'])
                self.md.start_migration(self.migration_pool, id)

        else:
            Trace.alarm(
                e_errors.ALARM, "Potential data loss. No policy for %s" %
                (new_content,))

        Trace.trace(
            10, "handle_cache_written: POLICY:%s" %
            (policy['policy'],))
        return True
...
def do_work():
    ...
    while True:
        ...
        if thread_is_running(t_n):
            ...
            Trace.log(
                e_errors.INFO, "dispatcher %s (re)starting %s" %
                (dispatcher.name, t_n))
    ...
    Trace.alarm(
        e_errors.ALARM, "dispatcher %s finished (impossible)" %
        (intf.name,))
