...
class UDP2amq(UDPServer):
    ...
    def __init__(self, udp_srv_addr, receive_timeout=60., use_raw=None, amq_broker=(
            "localhost", 5672), target_addr=None, auto_ack=True):
        '''
        Constructor
    ...
    def serve_udp(self):
        ...
        try:
            while not self.shutdown:
                ...
                if isinstance(ticket, dict) and (ticket.get(
                        "r_a", None) or ticket.get("ra", None)):
                    reply_to = self.myaddr
                else:
                    reply_to = None

                self.trace.debug(
                    "serve_udp(): to=%s reply_to=%s",
                    self.target_addr,
                    reply_to)
                ...
                try:
                    ...
                    self.log.error(
                        "serve_udp() : sending message, error=%s", e)
                    continue
    ...
    def _fetch_qpid_reply(self):
        try:
            ...
            # make sure msg is not None, otherwise it will ack all messages in
            # session
            if self.auto_ack and msg:
                try:
                    ...
                    # @todo : close() snd.
                    self.trace.debug(
                        "_fetch_qpid_reply(): sending acknowledge")
            ...
            self.log.error(
                "_fetch_qpid_reply(): fetching message, error=%s", e)
            return None
    ...
    def serve_qpid(self):
        ...
        try:
            while not self.shutdown:
                ...
                # relay ticket to UDP
                # convert lists to tuples to make getsockaddrarg() happy,
                # replace unicode by ascii str
                ticket = normalize_ticket(reply.content)

                try:
                    self.trace.debug(
                        "serve_qpid()  - received reply, ticket %s", ticket)

                    if isinstance(ticket, dict) and ticket.get("r_a", None):
                        self.reply_to_caller(ticket)
                    elif isinstance(ticket, dict) and ticket.get("ra", None):
                        self.reply_with_address(ticket)
                    else:
                        self.log.warning(
                            "serve_qpid()  - no reply address in reply, ticket %s.", ticket)
                        continue
                except BaseException:
                    exc, emsg = sys.exc_info()[:2]
                    self.log.error(
                        "serve_qpid()  - forwarding reply to udp client, (exception, msg)= %s %s",
                        str(exc),
                        str(emsg))
        finally:
...
if __name__ == "__main__":   # pragma: no cover
    ...
    u2a_srv = UDP2amq(('dmsen06.fnal.gov', 7700), use_raw=1, amq_broker=(
        "dmsen04.fnal.gov", 5672), target_addr="udp_relay_test")
