...
class Migrator(dispatching_worker.DispatchingWorker,
    ...
    def load_configuration(self):
        ...
        if self.clustered_configuration:
            ...
            self.migration_worker_configuration['server']['queue_work'] = work_queue
            self.migration_worker_configuration['server']['exchange_work'] = work_queue.split('.')[0]
        else:
            #self.migration_worker_configuration['server']['queue_work'] = "%s; {create: always}" % (
            #    work_exchange,)
            self.migration_worker_configuration['server']['queue_work'] = work_exchange
        ...
        self.migration_worker_configuration['server']['queue_in'] = self.queue_in_name  # migrator input control queue
    ...
    def __init__(self, name, cs, logclient=None):
        ...
        self.logger, self.tracer = rabbitmq_cache.en_logging.en_logging.set_logging(
            self.log_name)
        Trace.log(e_errors.INFO, "create %s server instance, client instance %s" %
                  (self.name,
        ...
        self.proc_statuses = self.manager.dict()
        self.status_dict = {}
    ...
    def _change_proc_counter(self, increment=0):
        ...
        Trace.trace(10, "proc_counter: {}".format(self.proc_counter.value))
    ...
    def pack_files(self, request_list):
        ...
        for component in request_list:
            ...
            if os.path.exists(cache_file_path):
                ...
                Trace.alarm(
                    e_errors.ERROR,
                    "File {} ({}) does not exist in cache. It will be skipped, but please investigate".format(
                        cache_file_path, component['bfid']))
    ...
    def write_to_tape(self, request, mq):
        ...
        # save correlation_id
        request_list = rq.content['content']['file_list']
        Trace.trace(10, "write_to_tape: request_list %s" % (request_list))
        if not request_list:
            raise e_errors.EnstoreError(
                errno.ENOENT, "Write to tape failed: no files to write", e_errors.NO_FILES)
        ...
        if len(request_list) == 0:
            ...
            status_message = mw_client.MWRArchived(orig_msg=rq)
            Trace.trace(10, "putting into local queue {}".format(status_message))
            try:
                mq.put(status_message)
            except Exception as e:
                Trace.trace(10, "sending reply, message {} exception {}".format(status_message,e))
        ...
        if write_enabled_counter != len(request_list):
            ...
            status_message = mw_client.MWRStatus(orig_msg=rq, content=content)
            Trace.trace(10, "putting into local queue {}".format(status_message))
            try:
                mq.put(status_message)
            except Exception as e:
        ...
        Trace.log(
            e_errors.INFO, "write_to_tape: id %s packed_file %s" %
            (rq.correlation_id, packed_file,))
        if packed_file:
        ...
        if res['status'][0] in (e_errors.OK, e_errors.TOO_MANY_FILES):
            ...
            raise e_errors.EnstoreError(
                errno.EFAULT, "Write to tape failed: %s" %
                (res['status'][1],), res['status'][0])

        # The file from cache was successfully written to tape.
        # Remove temporary file
        self.clean_up_after_write(src_file_path)
        Trace.trace(10, "Will make MWRArchived {}".format(rq))
        status_message = mw_client.MWRArchived(orig_msg=rq)
        Trace.trace(10, "putting into local queue {}".format(status_message))
        try:
            mq.put(status_message)
    ...
    def purge_files(self, request, mq):
        ...
        status_message = mw_client.MWRPurged(orig_msg=rq)
        Trace.trace(10, "putting into local queue {}".format(status_message))
        try:
            mq.put(status_message)
        except Exception as e:
    ...
    def read_from_tape(self, request, mq):
        ...
        content= rq.content['content']
        request_list = content['file_list']
        ...
        if len(files_to_stage) != 0:
            ...
            if not rc:
                # stage has failed
                return False
        status_message = mw_client.MWRStaged(orig_msg=rq)
        Trace.trace(10, "putting into local queue {}".format(status_message))
        try:
            mq.put(status_message)
        except Exception as e:
    ...
    def run_mw_request(self, message):
        ...
        Trace.trace(10, "APPENDING {} {}".format(status_dict, dir(status_dict)))
        with self.proc_lock:
            self.proc_statuses[message.correlation_id] = status_dict  # and append it to shared list
        ...
        try:
            ...
            content = {"migrator_status":
                       ...
                       "name": self.name}
            status_message = mw_client.MWRStatus(orig_msg=message, content=content)
            try:
                #self._send_reply(status_message)
                sndr.send(status_message)
            except Exception as e:
                self.trace.exception("sending reply, exception %s", e)
        ...
        try:
            ...
            msg = None
        if msg:
            Trace.trace(10, "SEND REPLY FROM THREAD {} {}".format(msg, dir(msg)))
            Trace.trace(10, "DETAIL: corr_id {} content {} properties {}".format(msg.correlation_id, msg.content, msg.properties))
            sndr.send(msg)
            #self._send_reply(msg)
            
        q.close()
        Trace.trace(10, "REMOVING {} {} {}".format(message.correlation_id, self.proc_statuses[message.correlation_id], self.proc_statuses))

        with self.proc_lock:
            try:
                val = self.proc_statuses.pop(message.correlation_id, None)
            except Exception as e:
                Trace.trace(10, "Exception removing {} from {}".format(message.correlation_id, self.proc_statuses))
        Trace.trace(10, "LEFT {}".format(self.proc_statuses))
    ...
    def process_mw_request(self, message, queue, status_dict):
        ...
        #mw.MigrationWorker.__init__(
        #    self, self.my_name, self.migration_worker_configuration)
    ...
    def handle_request(self, message):
        ...
        if proc_counter >= self.max_proc:
            ...
            return False
        Trace.trace(10, "handle_request msg structure: {}".format(dir(message)))
        Trace.trace(10, "handle_request correlation_id: {}".format(message.correlation_id))
        Trace.trace(10, "handle_request properties: {}".format(message.properties))
        Trace.trace(10, "handle_request content: {}".format(message.content))
        if message.correlation_id in self.work_dict:
        ...
        if request_type in (mt.MWC_ARCHIVE, mt.MWC_PURGE, mt.MWC_STAGE):
            ...
            try:
                ...
                Trace.trace(10, "Confirmation sending exception {}".format(e))
                self.trace.exception("sending reply, exception {}".format(e))
                return False
    ...
    def get_status(self, ticket):
        ...
        for key, item in self.proc_statuses.items():
            Trace.trace(10, "get_status: item %s %s" % (item, type(item)))
            if len(item):
        ...
        Trace.trace(10, "get_status: replying with {}".format(ticket))
