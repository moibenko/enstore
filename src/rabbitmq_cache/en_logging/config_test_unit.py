...
# $Id$
##############################################################################
import rabbitmq_cache.en_logging.handlers as handlers
import logging
...
def set_logging_console(name=None, full_name=None):
    ...
    # both channels can be configured to report to enstore's Trace.log and
    # Trace.trace
...
def set_logging_enstore(name=None, conf_srv=None, full_name=None):
    ...
    fmt = logging.Formatter(
        "%(levelname)s chan=%(name)s module=%(module)s file=%(filename)s %(lineno)d %(message)s")
    fmt_en = logging.Formatter(
        "%(asctime)s %(name)s %(levelname)s %(message)s")

    # Log settings
    lh = handlers.EnLogHandler(name)
    lh.setFormatter(fmt)
    ...
    # Trace settings
    th = handlers.EnTraceHandler(name)
    th.setFormatter(fmt)
