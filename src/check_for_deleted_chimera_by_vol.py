...
def main(intf):
    ...
    tape_info = ifc.inquire_vol(intf.vol)
    
    print("{} reported active files {} deleted_files {}".format(intf.vol,
                                                                tape_info['active_files'],
                                                                tape_info['deleted_files']))
    #sys.exit(0)
    ...
    for key, value in namespaceDictionary.items():
        ...
        try:
                ...
                for row in res:
                    ...
                    if not ilocation and itype == 2:
                        ...
                        if e_errors.is_ok(finfo):
                            bfid = finfo.get('bfid')
                            delete_candidates.append({'bfid': bfid, 'pnfsid': pnfsid})
                    elif ilocation == '\n':
                        pass
            ...
            for item in [cursor, db, connectionPool]:
                if item:
                    item.close()
    print("{} files to delete {}".format(intf.vol,len(delete_candidates)))
    print("{} removed vs active {:.2f}%".format(intf.vol,
                                            len(delete_candidates)/tape_info['active_files']*100))
    if len(delete_candidates):
        with open("{}.todelete".format(intf.vol), 'w') as f:
