...
def diff_fields_and_values(s1, s2):
    ...
    #
    key1 = [k for k in list(s1.keys()) if k not in ('active_files',
                                              'deleted_files',
                                              'unknown_files',
                                              'active_bytes',
                                              'deleted_bytes',
...
def get_fields_and_values(s):
    a = ','
    b = ','
    return a.join(list(s.keys())), b.join(map(str_value, list(s.values())))
...
class DbTable(object):
    ...
    def __setitem__(self, key, value):
        ...
        if len(res) == 0:        # insert
            ...
            if d:        # only if there is any difference
                query = dbaccess.generate_update_query(self.table, list(d.keys()))
                query += " WHERE {}=%s".format(self.pkey)
                v = list(d.values())
                v.append(key)
                self.dbaccess.update(query, tuple(v))
    ...
    def update_record(self, key, value):
        ...
        """
        v1 = self.import_format(value)
        query = dbaccess.generate_update_query(self.table, list(v1.keys()))
        query += " WHERE {}=%s".format(self.pkey)
        v = list(v1.values())
        v.append(key)
        self.dbaccess.update(query, tuple(v))
...
class FileDB(DbTable):
    ...
    def insert_new_record(self, key, value):
        ...
        query = query.format(self.table, ",".join(list(v1.keys())))
        ...
        res = self.dbaccess.update_returning_result(query, list(v1.values()))
        if self.auto_journal:
            self.jou[key] = value
...
class VolumeDB(DbTable):
    ...
    def import_format(self, s):
        sts = s['volume_family'].split('.') if 'volume_family' in s else None
        data = {}
        if 'blocksize' in s:
        ...
        if 'si_time' in s:
            data['si_time_0'], data['si_time_1'] = list(map(
...
if __name__ == "__main__":   # pragma: no cover
    ...
    print(list(v.keys()), len(v))
    random_label = list(v.keys())[number]
    volume = v[random_label]
    print(volume)
    local_copy = copy.deepcopy(volume)
    v[list(v.keys())[number]] = local_copy  # update
    label = 'XXXX01'
    del v[label]
