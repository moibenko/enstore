...
def generate_insert_query(table_name, keys):
    ...
    INSERT INTO {} ({}) VALUES ({})
    """
    query = query.format(table_name, ",".join(
        keys), (("%s,") * len(keys))[:-1])
    return query
...
def generate_update_query(table_name, keys):
    ...
    UPDATE {} SET {}
    """
    query = query.format(table_name, "=%s,".join(keys) + "=%s")
    return query
...
class DatabaseAccess(object):
    ...
    def insert(self, s, record=None):
        if record:
            q = generate_insert_query(s, list(record.keys()))
            return self.update(q, list(record.values()))
        else:
    ...
    def insert_returning_result(self, s, record=None):
        if record:
            q = generate_insert_query(s, list(record.keys()))
            return self.update_returning_result(q, list(record.values()))
        else:
