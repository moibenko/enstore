...
# define as globals, otherwise exec in Pyhton 3 does not work
outage = {}
offline = {}
override = {}
seen_down = {}
...
class HTMLExtraPages(object):

    def write(self, doc):
        if doc.extra_queue_pages:
            for extra_page_key in list(doc.extra_queue_pages.keys()):
...
class HTMLStatusFile(EnStatusFile, HTMLExtraPages, enstore_status.EnStatus):
    ...
    def write(self):
        if self.openfile:
            ...
            status_keys = list(self.text.keys())
            self.filelist = []
...
class EnDataFile(EnFile):
    ...
    def __init__(self, inFile, oFile, text, indir="", fproc=""):
        ...
        self.date = tmp.replace(enstore_constants.LOG_PREFIX, "")
    ...
    def check_line(self, line, start_time, stop_time, prefix):
        ...
        datetime = fields[0].strip()
        # remove the beginning file prefix if there is one
        if prefix:
            l = datetime.replace(prefix, "")
        else:
            l = datetime
...
class EnMountDataFile(EnDataFile):
    ...
    def __init__(self, inFile, oFile, text, indir="", fproc="", config={}):
        ...
        self.config = config
        self.servers = list(self.config.keys())
    ...
    def parse_line(self, line):
        ...
        if not self.is_null_mover(mover):
            if type == Trace.MSG_MC_LOAD_REQ.rstrip():
        ...
        for line in self.lines:
            ...
            if minfo and (not mcs or enstore_status.mc_in_list(
                ...
                self.data.append([minfo[MDEV], minfo[MPID], minfo[MVOLUME],
                                  minfo[0].replace(prefix, ""),
                                  minfo[MSTART], minfo[MDRIVEID]])
    ...
    def parse_data(self, line, mcs, prefix):
        ...
        if encp_line.valid:
            if not mcs or enstore_status.mc_in_list(encp_line.mc, mcs):
                ...
                self.data.append([etime.replace(prefix, ""),
...
class ScheduleFile(EnFile):
    ...
    def read(self):
        ...
        try:
            EnFile.open(self, 'r')
            if self.openfile:
                code = ''.join(self.openfile.readlines())
                exec(code, globals())
            else:
                outage = {}
                offline = {}
