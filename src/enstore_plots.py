...
def translate_drive_id(drive_id):
    ...
    drive_id = drive_id.replace('/', '-')
    return DRIVE_IDS.get(drive_id, drive_id)


def get_ctr(fields):
    return int(float(fields[1]) / float(fields[6]))
...
def sort_stamp_files(tmp_stamps):
    ...
    while i < num_stamps:
        if tmp_stamps[i][0].find(enstore_constants.MPH_FILE) == -1:
...
def ignore_file(fil, ignore):
    for ignore_string in ignore:
        if not fil.find(ignore_string) == -1:
...
def find_files(files, dir, ignore):
    ...
    ps_files = []
    for sfile in files:
        if ignore_file(sfile, ignore) == YES:
            continue
        if not sfile.find(enstore_constants.JPG) == -1:
            # this file has '.jpg' in it
            if not sfile.find(STAMP_JPG) == -1:
                # this is a postage stamp file
                tmp_stamps.append((sfile,
                                   os.stat("%s/%s" % (dir, sfile))[stat.ST_MTIME]))
            else:
                jpg_files.append((sfile,
                                  os.stat("%s/%s" % (dir, sfile))[stat.ST_MTIME]))
        elif not sfile.find(enstore_constants.PS) == -1:
...
class MpdMonthDataFile(EnPlot):
    ...
    def plot(self, total_mounts, total_mounts_type):
        ...
        for drive_type in list(total_mounts_type.keys()):
    ...
    def install(self, html_dir):
        ...
        for file in list(self.files_to_install.keys()):
...
class BpdGnuFile(enstore_files.EnFile):
    ...
    def write(self, outfile, ptsfile, total, meansize, xfers, read_xfers, write_xfers,
              total_written, lw=20):
        psfiler = outfile.replace(enstore_constants.BPD_FILE,
                                 enstore_constants.BPD_FILE_R)
        psfilew = outfile.replace(enstore_constants.BPD_FILE,
...
class BpdDataFile(EnPlot):
    ...
    def sum_data(self, data):
        ...
        for [xpt, ypt, type, mover, drive_id, sg] in data:
            ...
            for mover in list(self.movers_d.keys()):
    ...
    def install(self, dir):
        ...
        filew = self.name.replace(enstore_constants.BPD_FILE,
                                  enstore_constants.BPD_FILE_W)
...
class BpdMonthDataFile(EnPlot):
    ...
    def get_total_bpd(self, bpd_l):
        ...
        total_w_bpd = 0.0
        for bpd in bpd_l:
            fields = bpd.strip().split()
            if len(fields) > 1:
                total_bpd = total_bpd + float(fields[1])
                num_xfers = num_xfers + \
                    int(float(fields[1]) / float(fields[6]))
    ...
    def install(self, dir):
        ...
        filew = self.name.replace(enstore_constants.BPD_MONTH_FILE,
...
class TotalBpdGnuFile(enstore_files.EnFile):
    ...
    def write(self, outfile, ptsfile, total, meansize, xfers, max_nodes, total_written, lw=20):
        psfilew = outfile.replace(enstore_constants.TOTAL_BPD_FILE,
...
class TotalBpdDataFile(EnPlot):
    ...
    def install(self, dir):
        ...
        filew = self.name.replace(enstore_constants.TOTAL_BPD_FILE,
                                  enstore_constants.TOTAL_BPD_FILE_W)
        psfilew = "%s/%s%s" % (self.dir, filew, enstore_constants.PS)
        os.system("cp %s %s" % (psfilew, dir))
        convert_to_jpg(psfilew, "%s/%s" % (dir, filew))
