...
server_keys = list(server_map.keys())
...
class InquisitorMethods(dispatching_worker.DispatchingWorker):
    ...
    def get_server(self, name):
        if isinstance(name, list):
            name = name[0]
        servers = list(self.server_d.keys())
    ...
    def update_variables_from_config(self, config):
        ...
        if not self.inquisitor.config:
            ...
            self.update_exit(1)
        for skey in list(self.server_d.keys()):
    ...
    def clear_sent_stalled_mail(self, server_name, node, ff=""):
        if ff:
            ...
            keys = list(self.sent_stalled_mail.keys())
            for key in keys:
    ...
    def update_exit(self, exit_code):
        ...
        if self.server_status_file_thread.is_alive():
            self.server_status_file_thread.join()
        if self.er_thread.is_alive():
            self.er_thread.join()
        os._exit(exit_code)
    ...
    def make_encp_html_file(self, now):
        ...
        encplines = enstore_pg.acc_encp_lines(
    ...
    def check_for_bad_writes(self, server):
        ...
        bad_mover_keys = list(bad_movers.keys())
    ...
    def get_server_info(self, servers_just_done, success_servers):
        ...
        self.er_lock.acquire()
        servers = list(self.server_er_msg.keys())
        self.er_lock.release()
    ...
    def do_one_request(self):
        ...
        for func, time_data in list(self.interval_funcs.items()):
...
class Inquisitor(InquisitorMethods, generic_server.GenericServer):
    ...
    def __init__(self, csc, html_file="", update_interval=NOVALUE, alive_rcv_to=NOVALUE,
        ...
        for server_key in list(self.server_d.keys()):
            server = self.server_d[server_key]
            server.hung_interval = self.inquisitor.get_hung_interval(
                server.name)

        for key in list(self.config_d.keys()):
        ...
        for server in list(self.server_d.keys()):
