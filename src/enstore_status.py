...
class EncpLine(object):

    def bytes_to_mbytes(self, inbytes):
        return "%.2f" % (inbytes / 1024.0 / 1024.0,)
...
class SgLine(object):
    ...
    def __init__(self, line):
        ...
        [self.time, self.node, self.pid, self.user, self.status, self.server,
         self.text] = line.split(None, 6)
        if not self.text.find(enstore_constants.PENDING) == -1:
        ...
        self.sg = None
        try:
            dummy, sg = self.text.split(":")
            # The parsed string may not end with storage group
            self.sg = sg.strip().split(" ")[0]
        except ValueError:
            # the text was not in the right format so ignore it
            pass


class EnStatus(object):
    def __init__(self):
        self.text = ''
