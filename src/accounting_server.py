...
class Server(dispatching_worker.DispatchingWorker,
    ...
    def filler(self):
        ...
        try:
            ...
            SELECT_LAST_TIME = "select coalesce(max(unix_time), {}) from encp_xfer_average_by_storage_group".format(
                zero_time)
    ...
    def log_encp_xfer(self, ticket):
        ...
        try:
            ...
            if 'file_family' not in ticket:
                ...
                sg, ticket['file_family'], ticket['wrapper'] = v['volume_family'].split(".")
