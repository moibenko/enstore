...
class FileClient(info_client.fileInfoMethods,  # generic_client.GenericClient,
    ...
    def create_bit_file(self, file, timeout=RCV_TIMEOUT, retry=RCV_TRIES):
        ...
        ticket["fc"]["size"] = int(file["size"])
        ticket["fc"]["sanity_cookie"] = file["sanity_cookie"]
        ticket["fc"]["complete_crc"] = int(file["complete_crc"])
...
def do_work(intf):
    ...
    if ticket:
        ...
        if ticket['status'][0] == e_errors.OK:
            ...
            list(map(printer, ticket["children"]))
    elif intf.replay:
        if intf.replay > 1:
        ...
        if ticket['status'][0] == e_errors.OK:
            print("bfid %s: sanity_cookie %s, complete_crc %s" % (repr(bfid), ticket["sanity_cookie"],
                                                                  # keep L
                                                                  # suffix
                                                                  repr(ticket["complete_crc"])))
    elif intf.set_crcs:
        bfid, sanity_size, sanity_crc, complete_crc = intf.set_crcs.split(',')
