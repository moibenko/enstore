...
class EcronData(object):
    ...
    def get_result(self, name, node, start=None):
        if start:
            ...
            q = "(select start as time, 10 from event where name = '%s' and split_part('%s','.',1) and comment is null) union (select finish as time, status from event where name = '%s' and split_part('%s','.',1) and not finish is null and comment is null) order by time;" % (
