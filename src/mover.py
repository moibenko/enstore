...
# modes
READ, WRITE, ASSERT = list(range(3))
...
def total_memory():
    ...
    return int(total_mem_in_pages) * int(page_size)
...
class Buffer(object):
    ...
    def dump(self, f):
        ...
        for name, value in list(self.__class__.__dict__.items()) + list(self.__dict__.items()):
            v = value
            try:
                l = len(value)
    ...
    def stream_read(self, nbytes, driver):
        ...
        if isinstance(driver, (str, bytes)):
    ...
    def _getspace(self):
        ...
        if self._freelist:
            ...
            r = struct.pack("%ss" % (self.blocksize,), b'')
        self._lock.release()
        return r
...
def cookie_to_long(cookie):
    if isinstance(cookie, int):
        return int(cookie)
    if not isinstance(cookie, (str, bytes)):
        raise_(
...
Linux, IRIX, Solaris, Other = list(range(4))
...
class Mover(dispatching_worker.DispatchingWorker,
    ...
    def __setattr__(self, attr, val):
        ...
        self.__dict__[attr] = val
        Trace.trace(
            10, "set state old: %s new: %s %s" %
            (cur_val, state_name(val), self.__dict__['state_change_time']))
    ...
    def set_new_bitfile(self, fc_ticket, vc_ticket):
        ...
        if fc_ticket.get(
                # no redirection was needed
                'original_library') and fc_ticket['original_library'] == vc_ticket['library']:
    ...
    def watch_syslog(self):
        ...
        if self.syslog_match:
            try:
                cmd = "$ENSTORE_DIR/bin/match_syslog '%s'" % (
                    self.syslog_match)
                result = shell_command(cmd)
                if result:
                    for l in result:
    ...
    def update_tape_stats(self):
        ...
        try:
            ...
            self.bloc_loc = int(self.stats[self.ftt.BLOC_LOC])
            self.block_size = self.stats[self.ftt.BLOCK_SIZE]
            self.bot = self.stats[self.ftt.BOT]
            self.read_errors = int(self.stats[self.ftt.READ_ERRORS])
            self.write_errors = int(self.stats[self.ftt.WRITE_ERRORS])
    ...
    def update_stat(self):
        ...
        if self.stats_on and self.tape_driver and self.tape_driver.ftt:
            try:
                user_read = int(stats[self.ftt.USER_READ]) / 1024.
            ...
            try:
                user_write = int(stats[self.ftt.USER_WRITE]) / 1024.
            except TypeError as detail:
    ...
    def find_mc_drive_address(self, serial_num):
        ...
        :rtype: :obj:`tuple` drive element address in TS4500 (int), location (str), volume (str)
        """
        Trace.log(e_errors.INFO, 'find mc drive address for {}'.format(serial_num))
        if not serial_num:
    ...
    def start(self):
        ...
        if self.driver_type == 'NullDriver':
            ...
            Trace.log(e_errors.INFO, "statistics path %s" % (self.stat_file,))
            self.compression = self.config.get('compression', None)
            if self.compression and self.compression > 1:
            ...
            if self.state == IDLE:
                ...
                try:
                    ...
                    Trace.log(e_errors.INFO, "PROD_ID: %s, SN: %s" %(self.config['product_id'], self.config['serial_num']))
                except (self.ftt.FTTError, TypeError) as detail:
                    Trace.alarm(
                ...
                if use_local_mc or detect_mc_device:
                    ...
                    self.mc_device, self.mc_device_phys_location, volume_as_known_to_mc = self.find_mc_drive_address(
                        self.config['serial_num'])
                    Trace.log(e_errors.INFO, "MC device %s %s %s" %
                        (self.mc_device, self.mc_device_phys_location, volume_as_known_to_mc))
                ...
                if self.config['product_id'] in ("T10000C", "T10000D"):
                    ...
                    if not disable_AMC:
                        ...
                        try:
                            ...
                            Trace.alarm(e_errors.ERROR, 'Failed to set  "Allow Maximum Capacity": %s' %
                                        (str(detail),))
                            sys.exit(-1)
                        if not enabled:
                    ...
                    if status[0] == e_errors.OK:
                        ...
                        for i in range(3):
                            ...
                            if isinstance(v, dict) and 'status' in v and v['status'][0] == e_errors.OK:
                                ...
                                found = True
                                break
                            elif v['status'][0] in (e_errors.NO_VOLUME, e_errors.WRONG_FORMAT):
                                not_found_count += 1
                                ...
                                elif self.media_type and not_found_count == 2:
                                    if 'LTO' in self.media_type:
                                        s = self.media_type.split('TO')
                                        self.media_type = s[0]+s[1]
                                        self.current_volume = self.current_volume_initial + self.media_type
    ...
    def _reinit(self):
        ...
        if encp_dict:
            self.crc_seed = int(encp_dict.get("crc_seed", 1))
    ...
    def update_lm(self, state=None, reset_timer=None, error_source=None):
        ...
        if self.state in (IDLE, HAVE_BOUND):
            ...
            if n_thread and n_thread.is_alive():
                ...
                return
            elif t_thread and t_thread.is_alive():
                if t_in_state <= 1:
                    # skip sending lm _update
                ...
                if self.config['product_id'].find("DLT") == -1:
                    ...
                    if self.delay_alarm_for_tape_thread:
                        ...
                        Trace.trace(10, 'delaying alarm for tape thread running in %s for %s s' % (
                            # repeat for stdio
                            state_name(self.state), t_in_state))
                        time.sleep(1)
                    ...
                    if self.bytes_read_last == self.bytes_read:
                        if self.mode in (WRITE, ASSERT):
                            ...
                            Trace.log(
                                ...
                                (n_thread.is_alive(),
                                 t_thread.is_alive()))  # Remove this when problem is fixed !!!!!
                            # see what thread is active
                            if n_thread.is_alive() and not t_thread.is_alive():
                ...
                if (int(now - buffer_empty_t) >
                    ...
                    Trace.log(e_errors.INFO, "write:now %s t %s max %s empty %s defer %s br %s btr %s" %
                              (now, buffer_empty_t,
                               ...
                               defer_write, self.bytes_read, self.bytes_to_read))
                    buffer_empty_t = now
                    Trace.trace(
    ...
    def assert_vol(self):
        try:
            ...
            Trace.log(
                e_errors.ERROR, 'exception during assert_vol: %s' %
                (str(e),))
        ...
        if status[0] != e_errors.OK:
            if status[0] == e_errors.READ_VOL1_READ_ERR and eod == 'none':
                ...
                self.send_client_done(
                    self.current_work_ticket, status[0], status[1])
                return
                ...
                for loc_cookie in keys:
                    ...
                    if not self.tr_failed or self.assert_return == e_errors.CRC_ERROR_IN_WRITE_CLIENT:
                        ...
                        while retries > 0:
                            if thread and thread.is_alive():
                                Trace.log(
                                    e_errors.INFO, "assert_vol: waiting for tape thread to finish")
                                retries -= 1
                                time.sleep(2)
                            else:
                                self.tr_failed = False
                        ...
                        if self.tr_failed:
                            Trace.log(
                                e_errors.ERROR,
                                'Volume assert failed due to not finished tape_thread')
                    if self.tr_failed:
                        stat = self._error
    ...
    def finish_transfer_setup(self):
        ...
        if bytes is None:
            ...
            self.bytes_to_transfer = int(bytes)
            self.expected_transfer_time = self.bytes_to_transfer / self.max_rate
...
n the drive" % (self.current_volume,))
    ...
    def update_after_writing(self):
        ...
        if self.driver_type == 'FTTDriver' and self.rem_stats:
            ...
            try:
                stats = self.tape_driver.get_stats()
                r2 = int(stats[self.ftt.REMAIN_TAPE]) * 1024
    ...
    def del_udp_client(self, udp_client):
        ...
        try:
            ...
            if not tsd:
                return
            for server in list(tsd.send_done.keys()):
                try:
                    tsd.socket.close()
                except BaseException:
                    pass
    ...
    def connect_client(self):
        ...
        try:
            ...
            hostinfo = socket.getaddrinfo(
                socket.gethostname(), None, caller_address_family)
        except Exception as detail:
            exc, msg, tb = sys.exc_info()
        ...
        try:
            ...
            if (not self.method) or (
                ...
                try:
                    ...
                    # get an ISCONN error.
                    if detail.errno == errno.EISCONN:
                        pass
                    # The TCP handshake is in progress.
                    elif detail.errno == errno.EINPROGRESS:
                        pass
                    else:
                ...
                Trace.trace(
                    10, 'connect_client: waiting on %s for %s' %
                    (self.control_socket, self.connect_to * self.connect_retry))
                ...
                try:
                    ...
                    """
                    Trace.log(e_errors.INFO, "SENDING %s" % (ticket,))
                    rtn = callback.write_tcp_obj_new(
...
# self.dismount_volume(after_function=self.idle)
    ...
    def seek_to_location(self, location, eot_ok=0, after_function=None):
        ...
        Trace.trace(
            24, "seeking to %s, after_function=%s" %
        ...
        while retries > 0:
            ...
            if thread and thread.is_alive():
                Trace.log(
                    e_errors.INFO,
                    "seek_to_location: waiting for tape thread to finish")
                retries -= 1
                time.sleep(2)
            else:
                failed = 0
        ...
        if failed:
            Trace.log(
                e_errors.ERROR, 'seek to location %s failed due to not finished tape_thread' %
                (location,))
            return
        try:
...
class DiskMover(Mover):
    ...
    def create_volume_name(self, ip_map, volume_family):
        ...
        return ':'.join((ip_map, volume_family,))
    ...
    def finish_transfer_setup(self):
        ...
        self.bytes_to_transfer = int(fc['size'])
        self.bytes_to_write = self.bytes_to_transfer
        self.bytes_to_read = self.bytes_to_transfer
    ...
    def update_after_writing(self):
        ...
        if v['status'][0] == e_errors.NO_VOLUME:
            ...
            r2 = int(stats.f_bavail) * stats.f_bsize
    ...
    for ch in changers:
        host = os.listdir(
            os.path.join(
                path_to_classes,
                ch,
                # scsi devise is in format: 10:0:0:1
                'device/scsi_device'))[0].split(':')[0]
        sg = os.listdir(
            os.path.join(
                path_to_classes,
                ch,
