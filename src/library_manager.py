...
class Requests(object):
    ...
    self.trace_level, "Requests: do_one_request:get returned %s %s" %
     (request, client_address))
        Trace.trace(self.trace_level,
                    "Requests: do_one_request:interval functions %s" % (list(self.worker.interval_funcs.items())))
    ...
    self.trace_level,
     "Requests: serve_forever: done do_one_request")
            ...
            now = time.time()
            for func, time_data in list(self.worker.interval_funcs.items()):
    ...
    self.trace_level, "Requests:process_request: finished %s(%s)" %
     (function, ticket))
                ...
                if t2 - t >= 3.:
                    ...
                    """
                    Trace.trace(
    # leave this on dispatching worker level
    5,"process_mover_request: changing logging")
                    try:
                        if not self.lch:
...
class AtMovers(object):
    ...
    self.trace_level, "MOVER %s" %
     (self.at_movers[mover],))
            ...
            if 'external_label' in mover_info and mover_info['external_label']:
                ...
                vol_family = self.at_movers[mover]['volume_family']
            # vol_family = self.at_movers[mover]['volume_family']
            # self.sg_vf.delete(mover, self.at_movers[mover]['external_label'],
            # storage_group, vol_family)
     ...
     (self.dont_update,))
        ...
        if self.at_movers:
            try:
                ...
                for mover in list(self.at_movers.keys()):
                    Trace.trace(
        ...
        'time_in_state', 0))
                        ...
                        if time_in_state > self.max_time_in_other:
                            if state not in ['IDLE', 'ACTIVE', 'OFFLINE', 'HAVE_BOUND', 'SEEK', 'MOUNT_WAIT',
                                             'DISMOUNT_WAIT']:
                                add_to_list = 1
                            if (state == 'SETUP' and self.at_movers[mover].get(
                                'current_volume')):
    ...
    def get_active_movers(self):
        ...
        mv_list = []
        for key in list(self.at_movers.keys()):
...
class PostponedRequests(object):
    """
    ...
    def get(self):
        ...
        if self.rq_list:
            ...
            l = []
            remove_these = []
            for sg in list(self.sg_list.keys()):
                if sg in self.rq_list:
    ...
    def update(self, sg, deficiency=0):
        if sg in self.rq_list: del (self.rq_list[sg])
        if sg in self.sg_list:
...
class PostponedBoundRequests(object):
    """
    ...
    def put(self, mover_ticket):
        ...
        Trace.trace(
    self.trace_level, 'postponed_bound:put: %s' %
     (self.rq_list,))
    ...
    def get(self):
        ...
        Trace.trace(
    self.trace_level, 'postponed_bound:get: %s' %
     (self.rq_list,))
...
class LibraryManagerMethods(object):
    """
    ...
    def __init__(self,
       ...
       """
        ...
        self.volumes_at_movers = AtMovers(max_time_in_active=max_time_in_active,
                                          # to keep information about what
                                          # volumes are mounted at which movers
                                          max_time_in_other=max_time_in_other)
    ...
    def __send_regret(self, ticket):
        ...
        try:
            ...
            Trace.trace(self.trace_level + 10, "send_regret %s" % (1,))
            flags = fcntl.fcntl(control_socket.fileno(), fcntl.F_GETFL)
            Trace.trace(self.trace_level + 10, "send_regret %s" % (2,))
            fcntl.fcntl(control_socket.fileno(), fcntl.F_SETFL, flags | os.O_NONBLOCK)
            Trace.trace(self.trace_level + 10, "send_regret %s" % (3,))
            ...
            if 'route_selection' in ticket and ticket['route_selection']:
                ...
                try:
                    ...
                    Trace.log(e_errors.ERROR, "error sending to %s (%s)" %
                              (ticket['routing_callback_addr'], str(e)))
                    self.del_udp_client(u)
                    return 1
                if 'callback_addr' in x: ticket['callback_addr'] = x['callback_addr']
                Trace.trace(self.trace_level + 10, "encp replied with %s" %
                            (x,))
                self.del_udp_client(u)
            Trace.trace(self.trace_level + 10, "connecting to %s" %
            ...
            Trace.trace(self.trace_level + 10, "send_regret: calling write_tcp_obj_new")
    ...
    (w["fc"]["external_label"], w.get(
        'mover', None), w['unique_id']))
            # Trace.trace(self.trace_level+1,'get_work_at_movers. ticket info:
            # %s %s'%(w["fc"]["external_label"],  w.get('mover',None)))
    ...
    def is_volume_full_no_rec(self, v, min_remaining_bytes):
        ...
        if left < int(min_remaining_bytes * SAFETY_FACTOR) or left < MIN_LEFT:
    ...
    self.trace_level + 2, 'write_volumes %s' %
     (self.write_volumes,))
        ...
        required_bytes = max(int(size * SAFETY_FACTOR), MIN_LEFT)
    ...
    4, "fair_share: sg_exceeded %s" %
     (self.sg_exceeded,))
        ...
        pw_sgs = list(self.pending_work.storage_groups.keys())
            ...
            if len(tags) > 1:
                for key in tags:
                    if self.pending_work.get_sg(
                        key) in list(self.postponed_requests.sg_list.keys()):
     ...
     (rq.ticket,))
            ...
            if rq.work == "read_from_hsm":
                ...
                if self.continue_scan:
                    if key:
                        # rq = self.pending_work.get(key, next=1,
                        # active_volumes=active_vols,
    ...
    self.trace_level + 41,
     "next_work_any_volume: continue")
            # rq = self.pending_work.get(next=1, active_volumes=active_vols,
            # disabled_hosts=self.disabled_hosts)
            rq = self._get_request(requestor, self.pending_work.get, next=1, active_volumes=active_vols,
                                   disabled_hosts=self.disabled_hosts)
    ...
    vol_family,
     use_admin_queue=0)
                ...
                if not rq:
                    ...
                    # current_location, use_admin_queue=0)
                ...
                # disable retrival of HiPri requests as they were
                # already treated above
                # rq = self.pending_work.get(external_label, current_location,
                # use_admin_queue=0)
                rq = self._get_request(requestor, self.pending_work.get, external_label, current_location,
                                       use_admin_queue=0)
                Trace.trace(self.trace_level + 10, "next_work_this_volume: use label %s rq %s" %
    ...
    self.trace_level + 10,
     "next_work_this_volume: got here")
                    # rq = self.pending_work.get(vol_family, next=1,
                    # use_admin_queue=0, disabled_hosts=self.disabled_hosts)
...
class LibraryManager(dispatching_worker.DispatchingWorker,
    ...
    5, 'request_thread:thread finished %s %s %s' %
     (ticket['work'], ticket['mover'], dt))
        ...
        if dt >= 3.:
            Trace.trace(
    5,"request_thread:process_mover_request: changing logging")
            try:
                if not self.lch:
        ...
        if (function, ticket) in self.mover_rq_in_progress:
            Trace.trace(5, 'removing %s from %s'%
    ((function, ticket), self.mover_rq_in_progress))
        ...
        if 'storage_group' in ticket['vc']:
            if ticket['vc']['storage_group'] in self.allow_access:
                ...
                for host in self.allow_access[ticket['vc']['storage_group']]:
                    if re.search(
                        # host is in the list: acccess granted
                        '^%s' % (host,), host_from_ticket):
                        return 1
    ...
    def verify_data_transfer_request(self, ticket):
        ...
        if not error_detected and 'vc' in ticket and 'file_family_width' in ticket['vc']:
            try:
                ticket['vc']['file_family_width'] = int(
                    # ff width must be an integer
                    ticket['vc']['file_family_width'])
            except:
        ...
        if not error_detected:
            ...
            if rc:
                ...
                # Trace.trace(self.my_trace_level+100, "call back %s pending
                # callback %s"%(ticket["callback_addr"], rq.callback))
    ...
    def _mover_idle(self, mticket):
        ...
        if "current_time" in mticket:
            mover = mticket['mover']
            Trace.trace(11, "_mover_idle: active movers: %s" % (list(self.volumes_at_movers.at_movers.keys()),))
        ...
        if found:
            ...
            if method == 'read_tape_start':
                ...
                if mover_rq_unique_id:
                    ...
                    mover_rq_unique_id = "-".join(s1[:-1])
                s1 = work_at_mover_unique_id.split("-")
                work_at_mover_unique_id = "-".join(s1[:-1])
                Trace.trace(self.my_trace_level,
        ...
        Trace.log(e_errors.INFO, _format % \
                  (w["work"],
                   ...
                   mticket["mover"],
                   w["wrapper"]["uname"]))
        if 'reject_reason' in w: del (w['reject_reason'])
        self.pending_work.delete(rq)
    ...
    def mover_error(self, mticket):
        ...
        if 'returned_work' in mticket and mticket['returned_work']:
            ...
            if d['work'] == 'write_to_hsm':
                if 'external_label' in d['fc']: del (d['fc']['external_label'])
                if 'external_label' in d['vc']: del (d['vc']['external_label'])
