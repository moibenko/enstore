...
def main():
    ...
    if 'status' in result and result['status'][0] == e_errors.OK:
        ...
        for pool in keys:  # pools are keys of the dictionary
            if pool == 'migration_pool' and len(
                    list(result['pools'][pool].keys())) != 0:
                for k in list(result['pools'][pool].keys()):
                ...
                for k, value in result['pools'][pool].items():
                    policy = k.strip(".")
                    t = value.list_type
                    if t != "CACHE_WRITTEN":
                        continue
    ...
    if not files:
        ...
        for policy, poollists in list(files.items()):
            print("<h2><policy=<a id=\"%s\">%s</a></h2>" % (policy, policy,))
            for pool, list in poollists.items():
                files_in_pool_policies = files_pool_policy[policy][pool]
                Q1 = "select count(*), sum(size)/1024. as total from file where bfid in ('%s')" % ("','".join(files_in_pool_policies))
                res = select(dbinfo, Q1)
                count = int(res[0]["count"])
                size = int(res[0]["total"])
                ...
                for key, value in list.items():
                    ...
                    while (end < nfiles):
                        ...
                        f.crc,f.pnfs_id,f.pnfs_path,f.archive_status from file f, volume v where v.id=f.volume and f.bfid in ('%s')" % ("','".join(value[start:end]))
                        res = select(dbinfo, Q)
                        for row in res:
