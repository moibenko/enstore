...
def check_volume(vol):
    ...
    if 'comment' not in bvol:
        bvol['comment'] = ''
    parts = bvol['volume_family'].split('.')
    if len(parts) >= 3:
    ...
    if len(parts) > 3:
        p = '.'
        bvol['volume_family'] = p.join(parts[:3])
    if 'at_mover' in bvol:
        del bvol['at_mover']
    if 'status' in bvol:
        del bvol['status']
    ...
    if 'mounts' in bvol:
        del bvol['mounts']
    bvol['remaining_bytes'] = int(bvol['remaining_bytes'])
    ...
    for i in pbfids:
        ...
        if bfile['sanity_cookie'] != (None, None):
            bfile['sanity_cookie'] = (
                int(
                    bfile['sanity_cookie'][0]), int(
                    bfile['sanity_cookie'][1]))
        if isinstance(bfile['size'], type(1)):
            bfile['size'] = int(bfile['size'])
        if isinstance(bfile['complete_crc'], type(1)):
            bfile['complete_crc'] = int(bfile['complete_crc'])
