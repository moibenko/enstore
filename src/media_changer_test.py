...
class MediaLoaderMethods(dispatching_worker.DispatchingWorker,
    ...
    def DoWork(self, function, ticket):
        ...
        if ticket['function'] in (
            ...
            # print
            # ticket['function'],ticket['vol_ticket']['external_label'],ticket['vol_ticket']['system_inhibit'][1],
            # ticket['vol_ticket']['user_inhibit'][1],media_type
...
class AML2_MediaLoader(MediaLoaderMethods):
    ...
    def __init__(self, medch, max_work=7, csc=None):
        ...
        if 'RobotArm' in self.mc_config:   # error if robot not in config
            self.robotArm = self.mc_config['RobotArm'].strip()
        else:
            Trace.log(e_errors.ERROR,
                      "ERROR:mc:aml2 no robot arm key in configuration")
            self.robotArm = self.mc_config['RobotArm'].strip()  # force the exception
            return
    ...
    def cleanCycle(self, inTicket):
        ...
        if isinstance(libraryManagers, bytes):
            lm = libraryManagers
            library = libraryManagers.split(".")[0]
        elif isinstance(libraryManagers, list):
            lm = libraryManagers[0]
            library = libraryManagers[0].split(".")[0]
        else:
            Trace.log(
                e_errors.ERROR,
...
#                  print "nread=",nread, "line=",nl, l
            ...
            for l in resp:
                if async_date.match(l):
                    if l.find('Place cartridges in CAP') != -1 or \
                       l.find('Remove cartridges from CAP') != -1 or \
                       l.find('Library error, LSM offline') != -1 or \
                       l.find('Library error, Transport failure') != -1 or \
                       l.find('Library error, LMU failure') != -1 or \
                       l.find('LMU Recovery Complete') != -1 or \
                       l.find(': Offline.') != -1 or \
                       l.find(': Online.') != -1 or \
                       l.find(': Enter operation ') != -1 or \
                       l.find('Clean drive') != -1 or \
                       l.find('Cleaned') != -1:
                        if self.DEBUG:
                            print("DELETED:", l)
                        jonflag = 1
    ...
    def mount(self, volume, drive, media_type="", view_first=1):
        ...
        if media_type.find('READONLY') != -1:
            media_type = media_type.replace('READONLY', '')
            readonly = 1
        else:
            readonly = 0
        # build the command, and what to look for in the response
...
class Shelf_MediaLoader(MediaLoaderMethods):
    ...
    def status_message(self, s):
        if s in list(self.status_message_dict.keys()):
    ...
    def __init__(self, medch, max_work=1, csc=None):
        ...
        if fnstatus == 'OK' and fnstatusO == 'OK':
            index = self.localHost.find(self.ocsHost)
            if index > -1:
                self.cmdPrefix = ""
                self.cmdSuffix = ""
            else:
    ...
    def getOCSHost(self):
        ...
        fnstatus = 'OK'
        self.ocsHost = self.mc_config['OCSclient'].strip()
        index = self.ocsHost.find(".")
        if index == 0:
            fnstatus = 'ERRCfgHst'
        return fnstatus
    ...
    def allocateOCSdrive(self, drive):
        ...
        if stat == 0:
            ...
            if retval != '0':
                ...
                retstring = result[0]
                pos = retstring.find(" " + drive)
                if pos == -1:  # different drive was allocated
                    fnstatus = 'ERRAloDrv'
                    pos = retstring.find(" ")
                    if pos != -1:
                        wrongdrive = retstring[pos + 1:].strip()
                        Trace.log(
    ...
    def deallocateOCSdrive(self, drive):
        ...
        if stat == 0:
            ...
            if retval != '0':
                retstring = result[0]
                pos = retstring.find("drive is already deallocated")
