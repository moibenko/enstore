...
def page_25_test(driver):
    ...
    if res:
        res = int(res)
    if res == 1:
        amc = input("enter 0 or 1 to set AMC or just press Enter: ")
        if amc:
            amc = int(amc)
            if amc > 1 or amc < 0:
        ...
        lfa = True
        res = input("Set LFA? (y/n): ")
        if res and res[0] == 'n':
            lfa = False
        fsa = True
        res = input("Set FSA? (y/n): ")
        if res and res[0] == 'n':
            fsa = False
...
def scsi_verify_test(driver):
    rc = input("0 - standard verify, 1 - complete verify : ")
    ...
    while True:
        ...
        if test_number:
            ...
            if test_number == 1:
                ...
                sdca = input("set - 1, clear - 0: ")
                if sdca:
                    sdca = int(sdca)
                    t10000c_sdca(driver, sdca=sdca)
            elif test_number == 6:
                c = input("set - 1, clear - 0: ")
