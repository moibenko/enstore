...
class EventRelayMsg(object):
    ...
    def send(self, sock, event_relay_addr):
        sock.sendto(self.message().encode(), (event_relay_addr))
...
class EventRelayNotifyMsg(EventRelayMsg):
    ...
    def decode(self, msg):
        ...
        self.host, self.port, self.msg_types = self.extra_info.split(
                                                            MSG_FIELD_SEPARATOR, 2)
        self.msg_type_l = self.msg_types.split(MSG_FIELD_SEPARATOR)
...
class EventRelayUnsubscribeMsg(EventRelayMsg):
    ...
    def decode(self, msg):
        self.type, self.extra_info = decode_type(msg)
        self.host, self.port = self.extra_info.split(MSG_FIELD_SEPARATOR, 1)
...
class EventRelayAliveMsg(EventRelayMsg):
    ...
    def decode(self, msg):
        self.type, self.extra_info = decode_type(msg)
        params = self.extra_info.split(MSG_FIELD_SEPARATOR, 3)
        self.host = params[0]
        self.port = params[1]
...
class EventRelayClientMsg(EventRelayMsg):
    ...
    def decode(self, msg):
        self.type, self.extra_info = decode_type(msg)
        self.host, self.work, self.file_family, self.more = self.extra_info.split(MSG_FIELD_SEPARATOR, 3)
...
class EventRelayStateMsg(EventRelayMsg):
    ...
    def decode(self, msg):
        self.type, self.extra_info = decode_type(msg)
        self.short_name, self.state_name = self.extra_info.split(MSG_FIELD_SEPARATOR, 1)
...
class EventRelayTransferMsg(EventRelayMsg):
    ...
    def decode(self, msg):
        ...
        self.short_name, self.bytes_read, \
            self.bytes_to_read = self.extra_info.split(MSG_FIELD_SEPARATOR, 2)
...
class EventRelayDisconnectMsg(EventRelayMsg):
    ...
    def decode(self, msg):
        self.type, self.extra_info = decode_type(msg)
        self.short_name, self.client_hostname = self.extra_info.split(
...
class EventRelayUnloadMsg(EventRelayMsg):
    ...
    def decode(self, msg):
        self.type, self.extra_info = decode_type(msg)
        self.short_name, self.volume = self.extra_info.split(
            MSG_FIELD_SEPARATOR, 1)
...
# Message format:  loading short_name volume
class EventRelayLoadingMsg(EventRelayMsg):
