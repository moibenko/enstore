...
TWO_G = 2 * int(ONE_G) - 1  # Used in int32()
MAX_FILE_SIZE = int(ONE_G) * 2 - 1    # don't get overflow
...
class EncpError(Exception):
    def __init__(self, e_errno, e_message, e_type, e_ticket={}):
        ...
        if e_errno in list(errno.errorcode.keys()):
            self.errno = e_errno
        else:
            self.errno = None
        ...
        elif isinstance(e_message, (bytes, str)):
...
def format_class_for_print(object, name):
    ...
    for var in dir(object):
        ...
        if type(ret) == bytes:  # if a string, make it look like one.
            ret = "'" + ret + "'"
    ...
    for i in range(0, len(dictionaries)):
        ...
        for key in list(dictionaries[i].keys()):
            ...
            if new.get(key, None) and \
               type(dictionaries[i][key]) == dict and \
               type(new[key]) == dict:
...
def get_enstore_pnfs_path(filepath):
    ...
    canonical_name = ".".join(socket.getfqdn().split(".")[1:])
...
def parse_layer_2(data):
    ...
    for line in data:
        ...
        if result is not None:
            ...
            dcache_crc_long = int(hex_dcache_string, 16)
        result = size_match.search(line)
        if result is not None:
            dcache_string = result.group().split("=")[1]
            try:
                dcache_size_long = int(dcache_string)
            except ValueError:
                dcache_size_long = None
...
def is_read(ticket_or_interface):
    ...
    if isinstance(ticket_or_interface, dict):
...
def all_requests_outstanding(requests):
    ...
    if type(requests) == list:
        ...
        files_left = 0
        for request_list in list(requests.values()):
...
def __get_csc(parameter=None, intf=None):
    ...
    if type(parameter) == dict:  # If passed a ticket with bfid.
    ...
    elif type(parameter) == tuple and len(parameter) == 2 \
    and type(parameter[0]) == bytes \
    and type(parameter[1]) == int:
        address = parameter
...
def __get_fcc(parameter=None):
    ...
    elif type(parameter) == dict:  # If passed a ticket with bfid.
        bfid = parameter.get('fc', {}).get("bfid",
                                           parameter.get("bfid", None))
    ...
    # for.
    for server in list(config_servers.keys()):
        try:
    ...
    __fcc = fcc
    if __fcc.server_address is not None and bfid != None:
        # If the fcc has been initialized correctly; use it.
...
def match_fc(match_fc_addr):
    if __csc is not None and __fcc != None and match_fc_addr != None:
...
def match_vc(match_vc_addr):
    if __csc is not None and __fcc != None and match_vc_addr != None:
...
def __get_vcc(parameter=None):
    ...
    elif type(parameter) == dict:  # If passed a ticket with volume.
        if 'fc' in parameter:
            volume = parameter.get('fc', {}).get('external_label',
                                                 parameter.get('volume', None))
    ...
    if __vcc and __vcc.server_address is not None and volume != None:
...
def clients(intf):
    ...
    # format_class_for_print(client['logc'],'logc'))
...
def is_null_media_type(volume_clerk_ticket, intf):
    if volume_clerk_ticket['wrapper'] == "null":
        ...
        for dictkey, value in list(csc.saved_dict.items()):
            if dictkey[-len(".mover"):] == ".mover" \
               and type(value) == dict \
               and value.get('driver', None) == "NullDriver":

                if type(value['library']) == list:
...
def print_data_access_layer_format(inputfile, outputfile, filesize, ticket):
    ...
    if type(outputfile) == list and len(outputfile) == 1:
        outputfile = outputfile[0]
    if type(inputfile) == list and len(inputfile) == 1:
    ...
    for i in range(0, len(request_list)):
        ...
        try:
            ...
            if not (e.override_deleted and
                ...
                # different, calling this function raises OSError exception.
                if is_read(e):
                    sfs.get_file_size(request['infile'])
                ...
                try:
                    data = sfs.readlayer(2, request['infile'])
            ...
            if len(list(rest.keys())) > 0:
                ...
                raise EncpError(None,
                                "Missing  metadata information: %s" % str(
                                    rest),
        ...
        except (EncpError, OSError, IOError):
            raise_(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2])
    ...
    for i in range(len(work_list)):
        ...
        # elif access_check(outputlist[i], os.F_OK) and dcache:
            ...
            try:
                ...
                layer1 = list([x.strip() for x in layer1])
                layer4 = list([x.strip() for x in layer4])
...
def open_data_socket(mover_addr, interface_ip=None):
    ...
    try:
        ...
        if msg.errno in  (errno.EISCONN, errno.EINPROGRESS):
            pass         # The TCP handshake is in progress.
...
def receive_final_dialog(control_socket):
    ...
    while True:
        try:
            ...
            done_ticket = {'status': (e_errors.NET_ERROR, detail)}
            break
...
def close_descriptors(*fds):
    ...
    for route in list(routes_to_close.keys()):
        try:
    ...
    ticket['work'] == "write_to_hsm":
        ...
        if lmd_name:
            ...
            if orig_library != t["vc"]["library"]:
                ticket.update(dict([i for i in iter(t.items()) if i[0] not in ("work")]))
...
def check_crc_layer2(done_ticket, encp_crc_1_seeded):
    ...
    encp_crc_long = int(encp_crc_1_seeded)
    encp_size_long = int(done_ticket['file_size'])
    ...
    if encp_size is None:
        ...
        ticket['status'] = (e_errors.UNKNOWN,
                            "Mover did not report how many bytes"
                                 " were transfered.")
        return ticket
    elif int(encp_size) != int(mover_size):
        ...
        if ticket['file_size'] is None:
            ...
            ticket['file_size'] = int(encp_size)
                    ...
                    if (encp_intf.get_bfid or encp_intf.get_bfids) and \
                       encp_intf.skip_pnfs:
                        ...
                        in_stat_info = (ticket['wrapper']['mode'],
                                        ...
                                        ticket['wrapper']['uid'],
                                        ticket['wrapper']['gid'],
                                        ticket.get('filesize', ticket.get('file_size',0)), # when merging head as SFA notices incosistency between filesize and file_size, this should take care of it
                                        fake_time,  # atime
                                        fake_time,  # mtime
                                        fake_time,  # ctime
                                        )
...
def handle_retries(request_list, request_dictionary, error_dictionary,
    ...
    if not max_retries:
        max_retries = enstore_constants.DEFAULT_ENCP_RETRIES
    if not max_submits:
        max_submits = enstore_constants.DEFAULT_ENCP_RESUBMISSIONS
    ...
    if local_filename:
        ...
        try:
            ...
            int(original_inode) != int(current_inode):
            ...
            if is_read(request_dictionary):  # read
                ...
                if infile in ["/dev/zero", "/dev/random", "/dev/urandom"]:
                    ...
                    current_size = stats[stat.ST_SIZE]
            if int(current_size) != int(original_size):
                lf_status = (e_errors.FILE_MODIFIED,
    ...
    if is_write(encp_intf) and type(pnfs_filename) == bytes \
    ...
    if isinstance(resubmits, (str, bytes)):
        resubmits = int(resubmits)
    if isinstance(max_submits, (str, bytes)):
...
def create_write_request(work_ticket, file_number,
    ...
    if ifullname in ["/dev/zero", "/dev/random", "/dev/urandom"]:
        bound_limits = [int(os.environ.get('ENSTORE_RANDOM_LB', 0)),
                        int(os.environ.get('ENSTORE_RANDOM_UB',
                                            2147483648))]
        bound_limits.sort()
        file_size = int(random.uniform(bound_limits[0], bound_limits[1]))
    # If we are reading from standard in don't set the size.
    elif ifullname.startswith("/dev/fd"):
        file_size = None
    else:
        file_size = int(istatinfo[stat.ST_SIZE])
    ...
    if file_number < (len(e.input) - 1):
        # In minutes.
        if isinstance(encp_el['delayed_dismount'], (int, float)):
            encp_el['delayed_dismount'] = max(3, encp_el['delayed_dismount'])
        else:
            encp_el['delayed_dismount'] = 3
    ...
    for i in range(len(request_list)):
        if e.put_cache or request_list[i].get('copy', None) is not None:
            ...
            try:
                ...
                request_list[i]['wrapper']['inode'] = int(pstat[stat.ST_INO])
            except OSError:
...
def write_to_hsm(e, tinfo):
    ...
    result_dict, work_ticket, request_ticket)
            ...
            if e_errors.is_non_retriable(result_dict):
                ...
                err_msg[_thread.get_ident()] = str(result_dict['status'])
    ...
    if e.output_file_family and e_errors.is_ok(done_ticket):
        ff = done_ticket["vc"]["file_family"].split(".")
...
def cookie_to_int(cookie):
    ...
    if not isinstance(cookie, str):
        raise_(
            ...
            (cookie, type(cookie)))
    if '_' in cookie:
        part, block, filename = cookie.split('_')
    else:
        filename = cookie
    if filename[-1] == 'L':
        filename = filename[:-1]
...
def verify_read_request_consistancy(requests_per_vol, e):
    ...
    # system.
    try:
        vol = list(requests_per_vol.keys())[0]
    ...
    for vol in vols:
        ...
        if request['outfile'] not in ["/dev/null", "/dev/zero",
            ...
            for quota in fs_quotas:
                ...
                sum_blocks = int(sum_size / fs_stats.f_bsize) + 1
...
def get_volume_clerk_info(volume_or_ticket, encp_intf=None):
    ...
    if vc_ticket is None:
        if type(volume_or_ticket) == bytes:
            volume_name = volume_or_ticket
        elif type(volume_or_ticket) == dict:
    ...
    if inhibit == e_errors.NOACCESS or \
       (inhibit == e_errors.NOTALLOWED and (encp_intf is not None and not encp_intf.override_notallowed)):
        if encp_intf is not None and encp_intf.check:
...
def create_read_request(request, file_number,
    ...
    if e.volume:
        ...
        if ifullname is None and sfsid != None:
            try:
...
def prepare_read_from_hsm(tinfo, e):
    ...
    for vol in list(requests_per_vol.keys()):
        if enstore_functions3.is_volume_tape(vol):
            requests_per_vol[vol].sort(key=get_cookie)
    ...
    if e.check:
        # Determine the name of the library.
        check_lib = list(requests_per_vol.values())[0][0]['vc']['library'] + \
            ".library_manager"
        try:
            return check_library(check_lib, e), listen_socket, \
...
def read_from_hsm(e, tinfo):
    ...
    done_ticket, listen_socket, udp_serv, rq_per_vol = prepare_read_from_hsm(tinfo, e)
    if not e_errors.is_ok(done_ticket) or e.check:
...
class EncpInterface(option.Interface):
    ...
    def __str__(self):
        ...
        the_list = list(self.encp_options.items())
    ...
    def parse_options(self):
        ...
        if type(self.max_resubmit) != bytes:
            pass  # Skip the int and None cases.  Only consider string cases.
...
def log_encp_start(tinfo, intf):
    ...
    try:
        os_info = tuple(s for s in os.uname())
    except OSError:
        os_info = ('', '', '', '', '')
    ...
    command_line = "Command line: %s" % (' '.join(intf.argv),)
...
def final_say(intf, done_ticket):
    ...
    try:
        ...
        if e_errors.is_ok(status) and err_msg[_thread.get_ident()]:
            ...
            err_msg_lock.acquire()
            err_msg[_thread.get_ident()] = str(status)
            err_msg_lock.release()
        ...
        err_msg_lock.acquire()
        err_msg[_thread.get_ident()] = str(("UNKNOWN", "UNKNOWN"))
        err_msg_lock.release()
...
def main(intf):
    ...
    #    Trace.do_print(x)
    for x in range(1, intf.verbose + 1):
        Trace.do_message(x)
...
def do_work(intf, main=main):
    ...
    try:
        ...
        err_msg_lock.acquire()
        err_msg[_thread.get_ident()] = message
        err_msg_lock.release()
    except BaseException:
