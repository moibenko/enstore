...
class Fs(object):  # pnfs_common.PnfsCommon, pnfs_admin.PnfsAdmin):
    ...
    def read_extended_attributes(self, name, filepath=None):
        ...
        if name is not None:
            ...
            if rtn is None:
                if len(ea_dict) == 1 \
                        and list(ea_dict.keys())[0][-len(name):] == name:
                    rtn = list(ea_dict.values())[0]
                else:
    ...
    def get_file_size(self, filepath=None):
        ...
        os_filesize = int(fstat[stat.ST_SIZE])
    ...
    def get_xreference(self, filepath=None):
        ...
        xinfo = list([x.strip() for x in value[:11]])
