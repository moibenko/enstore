...
def print_html(summary):
    ...
    if summary["stage_area"]["files"] > 0:
        print("<a href='enstore_sfa_show_cached_files_cgi'>",
    ...
    print("<a href='enstore_sfa_files_in_transition_cgi'> FILES IN TRANSITION </a>")
...
def main():
    ...
    for m in migrator_list:
        for k in ("stage_area",
                  ...
                  "archive_area"):
            areas[k] = m[k]
    volume_map = parse_mtab(list(areas.values()))
    ...
    if len(diff) != 0:
        ...
        print("<ul>")
        for i in diff:
            for key, value in list(areas.items()):
                if value == i:
                    print(
    ...
    for key, value in list(volume_map.items()):
    ...
    summary["data_area"]["size"] = int(res[0]["total"])
    ...
    summary["stage_area"]["size"] = int(res[0]["total"])
