...
def gethostinfo(verbose=0):
    ...
    if not GL_hostinfo:
        ...
        hostinfo1 = socket.getaddrinfo(hostname, None, socket.AF_INET)
        # hostinfo1 is like:
        ...
        if GL_hostinfo[2] == ["127.0.0.1"]:
            intf_ips = []
            for intf_dict in list(Interfaces.interfacesGet().values()):
...
def address_to_name(addr):
    ...
    global GL_known_ips
    if addr in list(GL_known_ips.keys()):
...
def name_to_address(name):
    ...
    if name in list(GL_known_names.keys()):
...
def update_domains(csc_or_dict):
    ...
    if isinstance(csc_or_dict, dict):
        domains = csc_or_dict
        system_name = domains.get('system_name', "default2")
    elif isinstance(csc_or_dict, object):
        domains = csc_or_dict.get('domains', 3, 3, update_domains=False)
        system_name = csc_or_dict.get_enstore_system(3, 3, update_domains=False)
    else:
        return
...
def _allow(addr):
    ...
    try:
        if not addr in GL_localhost_addresses:
            GL_localhost_addresses[addr] = (
                GL_host_name == socket.gethostbyaddr(addr)[0])
    ...
    for invalid_domains in list(invalid_domains_dict.values()):
    ...
    for valid_domains in list(valid_domains_dict.values()):
...
def interface_name(ip):
    ...
    if not ip:
        return
    if not GL_ifinfo or ip not in GL_ifinfo:
