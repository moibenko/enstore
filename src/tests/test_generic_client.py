...
class TestClientError(unittest.TestCase):
    def setUp(self):
        ...
        self.cerr3 = generic_client.ClientError(
            'some other thing  is wrong', 5, e_errors.WRONGPARAMETER)
    ...
    def test___str__(self):
        ...
        self.assertEqual(
            'ESRCH: [ ERRNO 3 ] No such process: something else is wrong',
            b_str)
        c_str = self.cerr3.__str__()
        self.assertEqual(
            'EIO: [ ERRNO 5 ] Input/output error: some other thing  is wrong',
            c_str)
...
class TestGenericClient(unittest.TestCase):
    def setUp(self):
        ...
        self.gc = generic_client.GenericClient(
            self.csc, self.name, flags=self.flags)
    ...
    def test__is_csc(self):
        ...
        csc_gc = generic_client.GenericClient(
            self.csc, enstore_constants.CONFIGURATION_CLIENT)
    ...
    def test_get_server_configuration(self):
        ...
        # These return values seem to be env dependent, and not match the
        # mocked return value above..
    ...
    def test_get_server_address(self):
        ...
        a, b = self.gc.get_server_address(
            enstore_constants.CONFIGURATION_SERVER)
    ...
    def test_send(self):
        ...
        self.assertEqual(
            expected, rslt, 'test_send expected %s got %s' %
            (expected, rslt))
        self._mocker.reset_mock()
        ...
        self.assertEqual(
            str(rslt),
            expected,
            'test_send socket.gaierror  test failed returned %s' %
            str(rslt))
        ...
        self.assertEqual(
            str(rslt),
            expected,
            'test_send socket.error  test failed returned %s' %
            str(rslt))
        ...
        self.assertEqual(
            str(rslt),
            expected,
            'test_send TypeError test failed returned %s' %
            str(rslt))
        ...
        self.assertEqual(
            str(rslt),
            expected,
            'test_send ValueError test failed returned %s' %
            str(rslt))
    ...
    def test_alive(self):
        ...
        self.assertEqual(
            rcvd, expected, "test_alive 1  rcvd=%s expected=%s" %
            (rcvd, expected))
        ...
        rcvd = str(self._mocker.call_args)
        self.assertEqual(
            rcvd, expected, "test_alive 2  rcvd=%s expected=%s" %
            (rcvd, expected))
    ...
    def test_handle_generic_commands(self):
        ...
        rcvd = str(self._mocker.call_args)
        err_msg = 'test_handle_generic_commands 1 expected=%s rcvd=%s' % (
            expected, rcvd)
        ...
        rcvd = str(self._mocker.call_args)
        err_msg = 'test_handle_generic_commands 2 expected=%s rcvd=%s' % (
            expected, rcvd)
        self.assertTrue(expected in rcvd, err_msg)
        ...
        rcvd = str(self._mocker.call_args)
        err_msg = 'test_handle_generic_commands 3 expected=%s rcvd=%s' % (
            expected, rcvd)
        self.assertTrue(expected in rcvd, err_msg)
        ...
        rcvd = str(self._mocker.call_args)
        err_msg = 'test_handle_generic_commands 4 expected=%s rcvd=%s' % (
            expected, rcvd)
        self.assertTrue(expected in rcvd, err_msg)
        ...
        rcvd = str(self._mocker.call_args)
        err_msg = 'test_handle_generic_commands 5 expected=%s rcvd=%s' % (
            expected, rcvd)
        self.assertTrue(expected in rcvd, err_msg)
    ...
    def test_dump(self):
        ...
        self.assertEqual(str(self._mocker.call_args),
                         expected, "test_dump error")
