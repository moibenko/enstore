...
TYPES_LIST = [type(0), type(0), type(0.0), type(list(range(3))), type(''), type([])]
...
class TestInterface(unittest.TestCase):
    ...
    def test_check_for_config_defaults(self, mock_trace):
        ...
        if not enstore_functions2._get_value(
    ...
    def test_contents(self, stdout_mock):
        ...
        for arg in dir(self.intf):
            if string.replace(arg, "_", "-") in list(self.intf.options.keys()):
                stdout_mock.write("%s %s :  %s \n " %
                                   (arg, type(getattr(self.intf, arg)), getattr(self.intf, arg)))

        # every other matched value
        for arg in dir(self.intf):
            if string.replace(arg, "_", "-") not in list(self.intf.options.keys()):
                stdout_mock.write("%s %s :  %s \n " %
                                   (arg, type(getattr(self.intf, arg)), getattr(self.intf, arg)))
                ...
                if isinstance('', etype):
                    for opt_dict in DICT_LIST:
                        ...
                        if self.intf.print_usage.call_count > 0:
                            ...
                            self.assertEqual(
    self.intf.print_usage.call_count, 1)
