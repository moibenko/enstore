...
class Label2(Label):
    ...
    def __init__(self, record_format, block_length, record_length,
        ...
        if int(block_length) <= BLOCK_LEN_LIMIT:
...
class EnstoreLargeFileWrapper(object):
    ...
    def __init__(self, ticket):
        ...
        if self.filename_len > MAXFILENAMELEN:
            ...
            raise e_errors.EnstoreError(errno.EMSGSIZE, msg, INVALIDLENGTH)
            # raise INVALIDLENGTH, "filename too long
            # (%s)"%(self.filename_len,)
        ...
        self.implementation_1 = self.implementation_1.replace(CVSREVISION, "")
    ...
    # according to the context this method is not needed
    # so just commnent it for now
    """
    def get_filename_objects(self, aList, aClass):
        if self.filename_len > FILENAMELEN1:
            ...
            while findex <= self.filename_len:
                aList.append(aClass(i.uppercase(),
...
def hdr_labels(ticket):
    ...
    return efile.hdr_labels().encode()
...
def eof_labels(file_checksum, sanity_cookie=0, sanity_cookie_size=0):
    ...
    if efile:
        ...
        return efile.eof_labels().encode()
    else:
        return b""
