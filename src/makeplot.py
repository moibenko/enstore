...
class file_filter(object):
    def __call__(self, list_element):
        ...
        return list_element.find(self.compare_value) + 1
...
class file_timestamp_filter(object):
    def __call__(self, list_element):
...
def filter_out_files(sys_name, log_dir, time_in_days):
    ...
    rate_log_files = list(filter(criteria, all_log_files))

    criteria.compare_value = sys_name
    sys_name_rate_log_files = list(filter(criteria, rate_log_files))
    ...
    recent_rate_log_files = list(filter(timestamp_criteria, sys_name_rate_log_files))
...
def write_smooth_file(smooth_file, scaled_file, smooth_num=40):
    ...
    while True:
        line = scaled_file.readline().strip()
        if not line:
            break
        split_line = line.split()
        if len(split_line) == 4:  # Older ratekeepers.
...
if __name__ == "__main__":   # pragma: no cover
    ...
    for group in groups:
        ps_filename[group] = ps_filename_template.replace('*', group + "_")
        jpg_filename[group] = jpg_filename_template.replace('*',
                                                            group + "_")
        jpg_stamp_filename[group] = jpg_stamp_filename_template.replace('*',
                                                                        group + "_")
