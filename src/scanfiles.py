...
def errors_and_warnings(fname, error, warning, information):
    ...
    if type(error) != list or \
        type(warning) != list or \
...
def get_layer_2(f):
    ...
    if layer2:
        ...
        try:
            ...
            if crc:
                l2['crc'] = int(crc.split(":")[1], 16)
            # l2['crc'] = long(crc_match.search(line2).group().split(":")[1],
            # 16)
...
def check_bit_file(bfid, bfid_info = None):
    ...
    prefix = " ".join([prefix, "...", file_record['external_label'],
...
def check_file(f, file_info):
    ...
    if layer2.get('hsm', None) == "no":
        ...
        # Check for size.
        real_size = int(f_stats[stat.ST_SIZE])
        layer2_size = layer2.get('size', None)
        if layer2_size is not None:
            layer2_size = int(layer2_size)  # Don't cast a None.
        ...
        for fname in [f, get_enstore_pnfs_path(f),
            ...
            if e_errors.is_ok(ffbp):
                ...
                try:
                    ...
                    found_time = int(match_result.group(0)[:-5])
    ...
    try:
        if not is_multiple_copy and not is_migrated_copy:
            ...
            # matches exactly.
            p_lc = layer4['location_cookie'].split('_')[2]
            f_lc = filedb['location_cookie'].split('_')[2]
            if p_lc != f_lc or \
    ...
    try:
        ...
        if int(layer4['size']) != int(filedb['size']):
            err.append('size(%d, %d, %d)' % (int(layer4['size']),
                                             int(real_size),
                                             int(filedb['size'])))
        elif real_size != 1 and int(real_size) != int(layer4['size']):
            err.append('size(%d, %d, %d)' % (int(layer4['size']),
                                             int(real_size),
                                             int(filedb['size'])))
        ...
        if layer2 and layer2.get('size', None) is None:
            ...
            if int(layer2['size']) != int(filedb['size']):
    ...
    try:
        if layer4.get('crc', "") == "":  # some do not have this field
            ...
            if int(layer4['crc']) != int(filedb['complete_crc']):
        ...
        if layer2 and layer2.get('size', None) is None:
            ...
            # environment.
            if int(layer2['crc']) != int(crc_1_seeded) and \
            int(layer2['crc']) != int(filedb['complete_crc']):
    ...
    try:
        ...
        if parent_id != parent_dir_id:
            if parent_id is not None and parent_dir_id != None:
...
def check_parent(f):
    ...
    # Actually perform this check that is common to all types of files.
    if parent_id is not None and parent_dir_id != None and \
