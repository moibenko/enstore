...
class Index(object):
    def __init__(self, db, dbHome, dbName, field):
        ...
        if os.path.isfile("%s/%s" % (self.dbHome, self.idxFile)):
            self.db = libtpshelve.open(dbEnv,
                                       self.idxFile,
                                       type='btree',
                                       dup=1,
                                       dupsort=1)
            ...
            if type(self.primary_db) != type(None):
                Trace.log(e_errors.INFO, "Building index %s ..."%
                          (self.idxFile))
                self.db = libtpshelve.open(dbEnv,
                                           self.idxFile,
                                           type='btree',
                ...
                Trace.log(e_errors.ERROR,
                          ...
                          (self.idxFile))
                raise_(indexError, "Index file %s does not exist" % (self.idxFile))
...
# junk           self.inx[name]=MyIndex(self.dbindex,name)
    ...
    def cursor(self, action,KeyOrValue=None):
        ...
        if action =="update":
            if self.auto_journal:
                if 'db_flag' in list(KeyOrValue.keys()):
    ...
    def __setitem__(self, key,value):
        ...
        try:
            v0 = self.db[key]
            for name in list(self.inx.keys()):
    ...
    def __getitem__(self, key):
        if self.cached and key in list(self.cache.keys()):
