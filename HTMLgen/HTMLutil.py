...
def parse_cern_line(line):
    ...
    url = line_elements[-1]
    if shape in ('rect', 'poly'):
        coord_list = [x[1:-1] for x in line_elements[1:-1]]
...
class Directory(UserList):
    ...
    def __cmp__(self, item):
        return HTMLgen.cmp(self.name, item)
    ...
    def tree(self):
        ...
        for item in self.data:
            if isinstance(item, HTMLgen.InstanceType):
                try:
                    list = list + item.tree()
...
def image_inventory(directory=os.curdir, prefix=None, flatten=None):
    ...
    for file in pix:
        ...
        inventory[file] = HTMLgen.Image(file,
                                        ...
                                        absolute=cwd)
        if flatten:
            inventory[file] = str(inventory[file])
    os.chdir(savedir)
    return inventory
...
def test_image_inventory():
    ...
    store_image_inventory(
        './image',
        'images.mar',
        'http://www.python.org/image')
...
def test_driver():
    ...
    for (question, function) in things2test:
        ...
        i = i + 1
    answer = input('Type numbers separated by commas: ')
    if answer:
        try:
            answers = list(map(
                int, list([x.strip() for x in answer.split(
                        ',')])))
            for item in answers:
