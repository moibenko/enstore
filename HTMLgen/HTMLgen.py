...
class SimpleDocument(BasicDocument):
    ...
    def __init__(self, resource=None, **kw):
        ...
        if resource:
            execfile(mpath(resource), self.__dict__)
        for name, value in list(kw.items()):
            setattr(self, name, value)
...
class SeriesDocument(SimpleDocument):
    ...
    def header(self):
        ...
        if self.banner:
            ...
            if bannertype in (TupleType, StringType):
                ...
                raise TypeError(
                    'banner must be either a tuple, instance, or string.')
    ...
    def footer(self):
        ...
        # self.datetime = time.strftime("%c %Z", t)    #not available in
        # JPython
        ...
        s.append('<br>\nGenerated: %s <BR>' %
                 self.date)  # can use self.datetime here instead
        s.append('<hr>\n</FONT>')
        return ''.join(s)
    ...
    def __init__(self, url='', text='', **kw):
        ...
        for item in list(kw.keys()):
            if item in self.__dict__:
                ...
                raise_(KeyError, repr(item) +
                       ' not a valid parameter for this class.')
...
class MailTo(object):
    ...
    def __str__(self):
        ...
        return '<A HREF="mailto:%s">%s</A>' % (
            self.antispam(self.address), self.text)
...
class List(collections.UserList):
    ...
    *columns* attribute to a number greater than one. This is
    ...
    def __init__(self, llist=None, **kw):
        self.data = []
        self.lvl = 0
        if llist:
            if isinstance(llist, type(self.data)):
                self.data[:] = llist
            else:
                self.data[:] = llist.data[:]
        for item in kw.keys():
    ...
    def sub_list(self, list):
        ...
        """
        self.lvl = self.lvl + 1
        if isinstance(list, InstanceType):
            try:
    ...
    def __init__(self, **kw):
        ...
        for item in list(kw.keys()):
            if item in self.__dict__:
                ...
                raise_(KeyError, repr(item) +
                       ' not a valid parameter of the Input class.')
        if Input.re_type.search(self.type) is None:
            raise_(KeyError, repr(self.type) +
                   ' not a valid type of Input class.')
...
class Select(collections.UserList):
    ...
    def __init__(self, data=None, **kw):
        ...
        for item in list(kw.keys()):
            if item in self.__dict__:
                ...
                raise_(KeyError, repr(item) +
                       ' not a valid parameter of the Select class.')
    ...
    def __str__(self):
        ...
        if self.onFocus:
            s.append(' onFocus="%s"' % self.onFocus)
        s.append('>\n')
        if isinstance(self.data[0], TupleType):
    ...
    def __init__(self, text='', **kw):
        ...
        for item in list(kw.keys()):
            if item in self.__dict__:
                ...
                raise_(KeyError, repr(item) +
                       ' not a valid parameter of the Textarea class.')

    def __str__(self):
        s = ['<TEXTAREA NAME="%s" ROWS=%s COLS=%s' %
    ...
    def __str__(self):
        ...
        for i in range(len(self.body)):
            for j in range(len(self.body[i])):
                if isinstance(self.body[i][j], StringType):
        ...
        if self.heading:
            if not self.colspan:
                if isinstance(self.heading[0], ListType):
...
class AbstractTagSingle(object):
...
class Image(AbstractTagSingle):
    ...
    def __str__(self):
        ...
        if self.attr_dict['usemap']:
            if isinstance(self.attr_dict['usemap'], InstanceType):
...
class BR(AbstractTagSingle):
    ...
    def __str__(self):
        ...
        if self.args and isinstance(self.args[0], IntType):
...
class AbstractTag(object):
    ...
    def __str__(self):
        ...
        for item in self.contents:
            if isinstance(item, StringType) and self.html_escape == 'ON':
        ...
        if not self.tagname:
            if self.contents[0] not in (1, 2, 3, 4, 5, 6):
                raise AttributeError(
                    "First arg of Heading must be int from 1 to 6.")
...
class InitialCaps(object):
    ...
    def __str__(self):
        ...
        wordlist = []
        for word in list:
            word = self.hi(word[0].upper()) + \
                self.lo(word[1:].upper())
    ...
    while True:
        ...
        if match:
            if collapse:  # skip chars outside group1
                ...
                i = match.end(1)
            endpoints.append(
                ('m', match.start(1), match.end(1)))  # text2Bmarked
            count = count + 1
        else:
...
def relpath(path1, path2):
    ...
    rel = (os.pardir + os.sep) * len(dirs1)
    # common point
    return rel + path2
