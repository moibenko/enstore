...
def isStringType(t): return isinstance(t, StringType)


def isTupleType(t): return isinstance(t, TupleType)
def isImageType(t): return hasattr(t, "im")
# from operator import isNumberType, isSequenceType # not in JPython 1.0.3


def isNumberType(t):
    type(t) in (IntType, float)
...
def _getdecoder(d, e, a, ac=()):
    ...
    if a is None:
        a = ()
    elif not isinstance(a, TupleType):
        a = (a,)
...
def _getencoder(mode, encoder_name, args, extra=()):
    ...
    if args is None:
        args = ()
    elif not isinstance(args, TupleType):
        args = (args,)
...
class Image(object):
    ...
    def save(self, fp, format=None, **params):
        ...
        try:
            ...
            SAVE[format.upper()](self, fp, filename)
...
def new(mode, size, color=0):
    ...
    if isTupleType(color):
        ...
        if len(color) == 3:
            ...
            color = color[0] + (color[1] << 8) + \
                (color[2] << 16) + (color[3] << 24)
        else:
            color = color[0]
