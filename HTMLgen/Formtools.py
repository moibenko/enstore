...
class InputTable(object):
    ...
    def __init__(self, entries=None, **kw):
        ...
        for item in list(kw.keys()):
            if item in self.__dict__:
                ...
                detail = "%s not a valid parameter of the %s class." % (
                    item, self.__class__)
    ...
    def append(self, *items):
        for item in items:
            ...
            if li == 1:
                self.entries.append(
                    (item[0], Input(name=item[0], size=self.defaultsize), ''))
            elif li == 2:
...
class RadioButtons(object):
    ...
    def __init__(self, items, **kw):
        ...
        self.selected = []
        for (item, value) in list(kw.items()):
    ...
    def __str__(self):
        ...
        for item in self.items:
            if item in self.selected:
                s.append(
                    str(Input(type=self.widgettype, name=self.name, checked=1)))
            else:
