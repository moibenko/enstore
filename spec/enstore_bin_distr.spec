...
%define _unpackaged_files_terminate_build 0 # to let install files outside of rpm installation dir
...
if [ ! -d $RPM_BUILD_ROOT/etc/udev/rules.d ]; then
        mkdir -p $RPM_BUILD_ROOT/etc/udev/rules.d
fi

cp $RPM_BUILD_ROOT/%{prefix}/etc/udev/rules.d/75-changer-aliases-generator.rules $RPM_BUILD_ROOT/etc/udev/rules.d/
echo INSTALL DONE
%pre
PATH=/usr/sbin:$PATH
...
%config(noreplace) /etc/udev/rules.d/75-changer-aliases-generator.rules
