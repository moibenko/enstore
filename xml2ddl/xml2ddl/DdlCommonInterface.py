...
class DdlCommonInterface(object):
    ...
    def renameColumn(self, strTableName, strOldName,
                     strNewName, strNewColType, diffs):
        info = {
            'table_name': self.quoteName(strTableName),
    ...
    def addColumnComment(self, strTableName, strColumnName,
                         strDesc, strColType, ddls):
        info = {
            'table': strTableName,
    ...
    def changeColumnComment(
            self, strTableName, strColumnName, strDesc, strColType, ddls):
        self.addColumnComment(
            strTableName,
            strColumnName,
            strDesc,
            strColType,
    ...
    def renameIndex(self, strTableName, strOldIndexName,
                    strNewIndexName, cols, diffs):
        self.deleteIndex(strTableName, strOldIndexName, diffs)
        self.addIndex(strTableName, strNewIndexName, cols, diffs)

    def changeIndex(self, strTableName, strOldIndexName,
                    strNewIndexName, cols, diffs):
        self.deleteIndex(strTableName, strOldIndexName, diffs)
        self.addIndex(strTableName, strNewIndexName, cols, diffs)

    # Relations
    def addRelation(self, strTableName, strRelationName, strColumn,
                    strFkTable, strFk, strOnDelete, strOnUpdate, diffs):
        info = {
    ...
    # Autoincrement
    def addAutoIncrement(self, strTableName, strColName,
                         strDefault, strPreDdl, strPostDdl):
        info = {
            'table_name': strTableName,
            'col_name': strColName,
    ...
    def dropAutoIncrement(self, strTableName, col, diffs):
        ...
        if self.params['has_auto_increment']:
            self.doChangeColType(
                strTableName,
                strColName,
                self.retColTypeEtc(col),
                diffs)
            return
    ...
    def changeColDefault(self, strTableName, strColumnName,
                         strNewDefault, strColType, diffs):
        info = {
            'table_name': self.quoteName(strTableName),
    ...
    # Column type
    def doChangeColType(self, strTableName, strColumnName,
                        strNewColType, diffs):
        info = {
            'table_name': strTableName,
    ...
    def renameView(self, strOldViewName, strNewViewName,
                   newDefinition, newAttribs, diffs):
    ...
    # Function stuff
    def renameFunction(self, strOldFunctionName,
                       strNewFunctionName, newDefinition, newAttribs, diffs):
        self.dropFunction(
            strOldFunctionName,
            newAttribs['arguments'].split(','),
            diffs)
        self.addFunction(strNewFunctionName, newAttribs['arguments'].split(
            ','), newAttribs['returns'], newDefinition, newAttribs, diffs)
    ...
    def addFunction(self, strNewFunctionName, argumentList,
                    strReturn, strContents, attribs, diffs):
        info = {
    ...
    def updateFunction(self, strNewFunctionName, argumentList,
                       strReturn, strContents, attribs, diffs):
    ...
    def retColTypeEtc(self, col):
        ...
        if strPrec:
            strRet = '%s(%s, %s)%s%s' % (
                strType, strSize, strPrec, strDefault, strNull)
        elif strSize:
