...
class DownloadXml(object):
    ...
    def downloadSchema(self, tableList=None, of=sys.stdout):
        ...
        for strTableName in tables:
            ...
            if self.options is None or 'getindexes' not in self.options or self.options[
                    'getindexes'] == True:
            ...
            if self.options is None or 'getrelations' not in self.options or self.options[
                    'getrelations'] == True:
                curTable['relations'] = self.db.getTableRelations(strTableName)

            for colRow in self.db.getTableColumns(strTableName):
                (strColumnName, type, attlen, precision,
                 attnotnull, default, bAutoIncrement) = colRow
                curCol = {
                ...
                strComment = self.db.getColumnComment(
        ...
        if self.options is None or 'getviews' not in self.options or self.options[
                'getviews'] == True:
            self.getViews(of)

        if self.options is None or 'getfunctions' not in self.options or self.options[
                'getfunctions'] == True:
...
# litvinse@fnal.gov
        for mangledName in list(dict([(i, 1) for i in mangledNames]).keys()):
    ...
    def dumpFunction(self, info, of):
        of.write(
            ...
            (self.doAttribs(
                info, [
                    'name', 'arguments', 'returns', 'language'])))
    ...
    def dumpTable(self, info, of):
        ...
        for col in info['columns']:
            of.write(
                ...
                (self.doAttribs(
                    col, [
                        'name', 'type', 'size', 'precision', 'null', 'default', 'key', 'desc', 'autoincrement'])))
        ...
        if 'relations' in info and len(info['relations']) > 0:
            ...
            for index in info['relations']:
                ...
                of.write(
                    ...
                    (self.doAttribs(
                        curInfo, [
                            'name', 'column', 'table', 'fk', 'ondelete', 'onupdate'])))
...
def parseCommandLine():
    ...
    if info['dbname'] is None or info['user'] is None:
        parser.print_help()
        sys.exit(-1)
