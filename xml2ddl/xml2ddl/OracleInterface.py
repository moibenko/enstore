...
class OracleDownloader(DownloadCommon):
    ...
    def getTables(self, tableList):
        ...
        if tableList and len(tableList) > 0:
            ...
            inTables = "AND upper(TABLE_NAME) IN ('%s')" % (
                "' , '".join([name.upper() for name in tableList]))
        else:
            inTables = ""
    ...
    def getTableColumns(self, strTable):
        ...
        for row in rows:
            ...
            if numsize is not None or numprec is not None:
                size = numsize
    ...
    def getColumnComment(self, strTableName, strColumnName):
        ...
        self.cursor.execute(
            strSql, {
                'TABLENAME': strTableName, 'COLUMNAME': strColumnName})
        rows = self.cursor.fetchall()
        if rows:
            return rows[0][0]
    ...
    def getTableIndexes(self, strTableName):
        ...
        for row in rows:
            ...
            self.cursor.execute(
                strSql, {
                    'tablename': strTableName, 'indexname': strIndexName})
            ...
            if bIsUnique == 'UNIQUE':
                ...
                self.cursor.execute(
                    strSql, {
                        'tablename': strTableName, 'colnames': ','.join(colList)})
    ...
    def getViews(self, viewList):
        ...
        if viewList and len(viewList) > 0:
            inViews = "AND VIEW_NAME IN ('%s')" % (
                "','".join([name.upper() for name in viewList]))
        else:
            inViews = ""
    ...
    def getFunctions(self, functionList):
        ...
        if functionList and len(functionList) > 0:
            inFunctions = "AND OBJECT_NAME IN ('%s')" % (
                "','".join([name.upper() for name in functionList]))
        else:
            inFunctions = ""
    ...
    def getFunctionDefinition(self, strSpecifiName):
        ...
        if rows:
            if len(rows) > 1:
                ...
                DATA_TYPE = rows[0][0]
                strReturn = DATA_TYPE
        return (strSpecifiName.lower(), parameters,
                strReturn, None, strDefinition)
