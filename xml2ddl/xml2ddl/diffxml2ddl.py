...
## Changed b A.N.Moibenko: p2p3 convresion.
# Why this was anyway class mem
def ddlSorter(a, b):
    orderDict = {
        'Create Table': 0,
        'Drop Table': 0,
...
class DiffXml2Ddl(object):
    ...
    def changeAutoIncrement(self, strTableName, old, new):
        ...
        if strOldAuto != strNewAuto:
            if strNewAuto == 'yes':
                # Hmm, if we created the column the autoincrement would already
                # be there anyway.
                pass
                # print "Add Autoincrement TODO"
            else:
                self.ddli.dropAutoIncrement(
                    strTableName, attribsToDict(old), self.diffs)
    ...
    def changeColType(self, strTableName, old, new):
        ...
        if self.normalizedColType(
                strNewColType) != self.normalizedColType(strOldColType):
            # print "Different\n%s\n%s" %
            # (self.normalizedColType(strNewColType),
            # self.normalizedColType(strOldColType))
            self.ddli.doChangeColType(
                strTableName,
                old.get('name'),
                strNewColType,
                self.diffs)
    ...
    def normalizedColType(self, strColTypeEtc):
        ...
        # I'm not trying to convert them to SQL9x which would make more sense,
        # perhaps.
        ...
        if strColTypeEtc == 'number' or strColTypeEtc == 'decimal':
            ...
            strColTypeEtc = strColTypeEtc.replace(
    ...
    def dropRelatedConstraints(self, strTable, strColumnName=None):
        if strColumnName is not None:
            self._dropRelatedConstraints(strTable, strColumnName)
        else:
            table = self.findTable(
                self.old_xml.getElementsByTagName('table'), strTable)
    ...
    def dropRelatedSequences(self, strTableName):
        if self.dbmsType == 'firebird' or self.dbmsType.startswith('postgres'):
            table = self.findTable(
                self.old_xml.getElementsByTagName('table'),
                strTableName)
            columns = table.getElementsByTagName('column')
            for column in columns:
    ...
    def _dropRelatedSequence(self, strTableName, col):
        if col.getAttribute('autoincrement').lower() == 'yes':
            self.ddli.dropAutoIncrement(
                strTableName, attribsToDict(col), self.diffs)
    ...
    def diffTableComment(self, tbl_old, tbl_new):
        ...
        if strOldComment != strNewComment and strNewComment:
            self.ddli.addTableComment(
                self.strTableName, strNewComment, self.diffs)
    ...
    def diffSomething(self, old, new, strTag, renameFunc,
                      changeFunc, addFunc, deleteFunc, findSomething, getName):
    ...
    def changeRelation(self, strTableName, old, new):
        ...
        if strColumnOld != strColumnNew or strTableOld != strTableNew or strFkOld != strFkNew or strDelActionOld != strDelActionNew or strUpdateActionOld != strUpdateActionNew:
            # print "Col %s != %s or\nTable %s != %s, or\nFk %s != %s or\nDel
            # %s != %s or\nUpdate %s != %s" % (strColumnOld, strColumnNew,
            # strTableOld, strTableNew, strFkOld, strFkNew, strDelActionOld,
            # strDelActionNew, strUpdateActionOld, strUpdateActionNew)
            self.dropRelation(strTableName, old)
    ...
    def renameView(self, ignore, old, new):
        ...
        if self.inDbms(new):
            self.ddli.renameView(
                old.getAttribute('name'),
                new.getAttribute('name'),
                strDefinition,
                attribs,
                self.diffs)
    ...
    def diffView(self, ignore, oldView, newView):
        ...
        if strOldContents != strNewContents:
            ...
            self.ddli.updateView(
                newView.getAttribute('name'),
                strDefinition,
                attribs,
                self.diffs)
    ...
    def createFunction(self, ignore, new, nIndex):
        ...
        argumentList = [arg.strip()
                        for arg in new.getAttribute('arguments').split(',')]

        if self.inDbms(new):
            self.ddli.addFunction(
                new.getAttribute('name'),
                argumentList,
    ...
    def dropFunction(self, ignore, func):
        if self.inDbms(func):
            self.ddli.dropFunction(
                func.getAttribute('name'),
                func.getAttribute('arguments').split(','),
                self.diffs)
    ...
    def sortDiffs(self):
        self.diffs.sort(key=functools.cmp_to_key(lambda a,b:ddlSorter(a,b)))
