...
class Xml2Ddl(object):
    ...
    def retColDefs(self, doc, strPreDdl, strPostDdl):
        ...
        for col in cols:
            colDefs.append(
                self.retColumnDefinition(
                    col, strPreDdl, strPostDdl))
    ...
    def retColumnDefinition(self, col, strPreDdl, strPostDdl):
        ...
        strRet = self.ddlInterface.quoteName(
            strColName) + ' ' + self.ddlInterface.retColTypeEtc(attribsToDict(col))
        ...
        if col.hasAttribute('autoincrement') and col.getAttribute(
            ...
            strRet += self.ddlInterface.addAutoIncrement(
                strTableName, strColName, col.getAttribute('default'), strPreDdl, strPostDdl)
    ...
    def createTable(self, doc):
        ...
        if doc.hasAttribute('desc') and self.dbmsType.startswith('mysql'):
            strTableStuff += " COMMENT=%s" % (
                self.ddlInterface.quoteString(doc.getAttribute('desc')))

        self.ddlInterface.addTable(
            rawTableName,
            colDefs,
            keys,
            strTableStuff,
        ...
        if doc.hasAttribute('desc'):
            self.ddlInterface.addTableComment(
                strTableName, doc.getAttribute('desc'), self.ddls)
    ...
    def addColumnComments(self, doc):
        ...
        for column in doc.getElementsByTagName('column'):
            if column.hasAttribute('desc'):
                self.ddlInterface.addColumnComment(strTableName, getColName(
                    column), column.getAttribute('desc'), 'TODO', self.ddls)
    ...
    for adict in dictionaryNode.getElementsByTagName('dict'):
        ...
        for nIndex in range(attrs.length):
            ...
            if strName == 'class':
                ...
                # Merge in the inherited values (multiple inheritance supported
                # with comma
                ...
                for aClass in classes:
                    if aClass not in dict:
                        ...
                        for key, val in list(dict[str(aClass)].items()):
...
def handleIncludes(ret):
    ...
    for include in includes:
        ...
        include.parentNode.insertBefore(
            new.documentElement.firstChild.nextSibling, include)
    ...
    for aNode in dictionaries:
        ...
        for column in ret.getElementsByTagName(str(strReplaceNodes)):
            ...
            if strClass in dict:
                ...
                for key, val in list(dict[strClass].items()):
