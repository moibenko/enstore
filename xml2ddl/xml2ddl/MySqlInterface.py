...
class MySqlDownloader(DownloadCommon):
        ...
        for col in fullcols:
            if len(col) == 7:
                ...
                (Field, Type, Collation, Null, Key,
                 Default, Extra, Privileges, Comment) = col
    ...
    def getTableIndexes(self, strTableName):
        ...
        for index in indexes:
            (Table, Non_unique, Key_name, Seq_in_index, Column_name, Collation,
             Cardinality, Sub_part, Packed, Null, Index_type, Comment) = index
            if Key_name == 'PRIMARY':
                bIsPrimary = True
            else:
                bIsPrimary = False
    ...
    def getTableRelations(self, strTableName):
        ...
        """
        re_ref = re.compile(
            r'\s*CONSTRAINT\s+`(\w+)`\s+FOREIGN KEY\s+\(([^)]+)\)\s+REFERENCES\s+`(\w+)`\s+\(([^)]+)\)( ON DELETE (?:[A-Z]+))?( ON UPDATE (?:[A-Z]+))?')
        ...
        for line in rows[0][1].split('\n'):
            ...
            if match:
                myCols = [str.strip()[1:-1]
                          for str in match.group(2).split(',')]
                fkCols = [str.strip()[1:-1]
                          for str in match.group(4).split(',')]
    ...
    def getViewDefinition(self, strViewName):
        ...
        if rows:
            ...
            reGetDef = re.compile(
                'CREATE (?:ALGORITHM=[^ ]+[ ])?VIEW [a-zA-Z0-9_$`\\.]+ AS (.*)')
    ...
    def getFunctionDefinition(self, strSpecifiName):
        ...
        strCreate = rows[0][2]
        # ex.CREATE FUNCTION `test`.`sales_tax`(subtotal real) RETURNS real
        # BEGIN    RETURN subtotal * 0.06;END
        re_createFunc = re.compile(
            r'\s* CREATE \s+ FUNCTION \s+ ([\w`\.-]*) \s* \( ([^)]*) \) \s+ RETURNS \s+ (\w+) \s+ (.*)',
        ...
        return (strSpecifiName, strParams, strReturns,
