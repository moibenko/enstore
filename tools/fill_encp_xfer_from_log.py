...
if __name__ == "__main__":
    ...
    if not acc:
        sys.stderr.write(
            "Failed to extract accounting parameters from configuration\n")
        sys.stderr.flush()
        sys.exit(1)
    ...
    for file in args:
        ...
        for line in f:
            ...
            if line.find('ENCP  Version') != -1:
                parts = line.split()
                node = parts[1].strip()
                version = line[line.find('Version') +
                               len('Version:'):line.find('OS')].strip()
                if node not in version_node_map:
                    version_node_map[node] = version
            if line.find('MSG_TYPE=ENCP_XFER') == -1:
                continue
            if counter >= 100:
                break
            parts = line.split()
            ...
            dict['src'] = parts[8]
            dict['dst'] = parts[10][:-1]
            if dict['node'] not in version_node_map:
            ...
            # counter = counter + 1
            q = "select * from encp_xfer where node='%s' and pid=%s and date='%s' and volume='%s'" % (
                dict['node'], dict['pid'], dict['date'], dict['volume'],)
            res = db.query(q)
            if res.ntuples():
